...
// Define types for entities that don't have proper types yet
...
// Subject Papers
export const getSubjectPapers = async () => {
  const res = await axiosInstance.get<ApiResonse<SubjectPaper[]>>(`${BASE}/subject-papers`);
  return res.data.payload;
};

export const getSubjectPapersWithPapers = async () => {
  const res = await axiosInstance.get<ApiResonse<SubjectPaper[]>>(`${BASE}/subject-papers/with-papers`);
  return res.data.payload;
};
export const getSubjectPaper = (id: number) => axiosInstance.get<SubjectPaper>(`${BASE}/subject-papers/${id}`);
export const createSubjectPaper = (data: Omit<SubjectPaper, 'id' | 'createdAt' | 'updatedAt'>) => axiosInstance.post<SubjectPaper>(`${BASE}/subject-papers`, data);
