...
import { 
  CheckCircle2, Book, IdCard, Users, ChevronRight, ChevronLeft, Shield, 
  AlertCircle, School, Building, 
  Heart, AlertTriangle,
  StepForward
} from "lucide-react";
import PersonalDetailsForm from "@/components/forms/student/PersonalDetailsForm";
import ParentDetailsForm from "@/components/forms/student/ParentDetailsForm";
import GuardianDetailsForm from "@/components/forms/student/GuardianDetailsForm";
...
import { StudentFormData, PersonalDetails, ParentDetails, GuardianDetails, HealthDetails, EmergencyContact, AcademicHistory, AcademicIdentifier, Accommodation } from "@/types/student";
...
const steps = [
  ...
  },
  {
    ...
    activeIcon: <Shield className="w-5 h-5 text-blue-600" />,
    content: GuardianDetailsForm,
    key: "guardianDetails"
  },
  {
...
export default function AddStudentPage() {
  ...
  const [formData, setFormData] = useState<StudentFormData>({
    ...
    guardianDetails: {} as GuardianDetails,
    healthDetails: {} as HealthDetails,
  ...
  const next = () => {
    if (current < steps.length - 1) {
      if (!completedSteps.includes(current)) {
        message.warning({
          content: 'Please complete the current step before proceeding',
  ...
  const handleStepClick = (stepIndex: number) => {
    ...
    if (completedSteps.includes(stepIndex) || stepIndex <= current) {
      ...
      message.warning({
        content: 'Please complete the previous steps first',
        icon: <AlertTriangle className="text-yellow-500" />
      });
    }
  };
  ...
  const handleFormSubmit = async (
    stepIndex: number, 
    data: PersonalDetails | ParentDetails | GuardianDetails | HealthDetails | EmergencyContact | AcademicHistory | AcademicIdentifier | Accommodation
  ) => {
    try {
      ...
      message.success({
        content: `${steps[stepIndex].title} saved successfully!`,
        duration: 2
      });
  ...
  const handleFinalSubmit = async () => {
    if (!isAllStepsCompleted) {
      message.error({
        content: 'Please complete all steps before submitting',
        icon: <AlertCircle className="text-red-500" />
      });
      return;
    }
    ...
    try {
      setIsSubmitting(true);
      // Here you would typically make an API call to save all form data
      console.log("Form Data:", JSON.stringify(formData, null, 2));
      message.success({
        content: 'Student registration completed successfully!',
        duration: 3
      });
      ...
      message.error({
        content: error instanceof Error ? error.message : 'Failed to submit form. Please try again.',
        icon: <AlertCircle className="text-red-500" />
      });
    } finally {
      setIsSubmitting(false);
                  ...
                  {steps.map((step, index) => (
                    <motion.div
                      ...
                      className={`flex items-start gap-4 cursor-pointer transition-all duration-200 ${
                        current === index ? "text-blue-600" : 
                        completedSteps.includes(index) ? "text-green-600" : "text-gray-500"
                      }`}
                    >
                      <div className={`flex items-center justify-center w-10 h-10 rounded-full flex-shrink-0 ${
                        current === index ? "bg-blue-100" : 
                        completedSteps.includes(index) ? "bg-green-100" : "bg-gray-100"
                      }`}>
                      ...
                      <div>
                        <h3 className={`font-medium ${
                          current === index ? "text-blue-600" : 
                          completedSteps.includes(index) ? "text-green-600" : "text-gray-700"
                        }`}>
                          {step.title}
                        </h3>
                        <p className={`text-sm ${
                          current === index ? "text-blue-400" : 
                          completedSteps.includes(index) ? "text-green-400" : "text-gray-400"
                        }`}>
                          {completedSteps.includes(index) ? 'Completed' : 
                           current === index ? 'In Progress' : 'Pending'}
                        </p>
                      </div>
                    </motion.div>
                  ))}
              ...
              <div className="flex justify-between mt-12 pt-6 border-t border-gray-200">
                    ...
                    {current === steps.length - 1 ? (
                      <motion.button
                        ...
                        disabled={!isAllStepsCompleted || isSubmitting}
                        className={`flex items-center gap-2 px-6 py-3 rounded-lg transition-all duration-200 ${
                          !isAllStepsCompleted || isSubmitting
                            ? "bg-gray-100 text-gray-400 cursor-not-allowed"
