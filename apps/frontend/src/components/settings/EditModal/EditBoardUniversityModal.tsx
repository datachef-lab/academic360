...
import { updateBoardUniversities } from '@/services/student-apis';
...
const EditBoardUniversityModal: React.FC<EditUserBoardUniversity> = ({ type,data, onClose }) => {
  const queryClient = useQueryClient();  
  const [formData, setFormData] = useState<BoardUniversity>({
        name: data.name || "",
        degree: data.degree || "",
 ...
 const updateData=useMutation({
      mutationFn:(formData:BoardUniversity)=>{
        if(data.id!==undefined){
          return updateBoardUniversities(formData,data.id);
        }
    ...
    <>
      ...
      <DialogContent className="w-[90vw] max-w-[900px] p-6  rounded-lg">
        ...
        <div className="grid grid-cols-2 gap-8 p-4">
          ...
          <div className="flex flex-col space-y-2">
            <Label htmlFor="email" className="pl-1">Code</Label>
            <div className="relative ">
          ...
          <div className="flex flex-col space-y-2">
            <Label htmlFor="phone" className="pl-1">Sequence</Label>
            <div className="relative ">
              <span className="absolute left-3 top-1/2 transform -translate-y-1/2"><Phone size={20}></Phone></span>
            <Input name="sequence" type='number'  value={formData.sequence||''} onChange={handleChange}  className={`w-full border border-gray-400 rounded-md pl-10 pr-3 py-2 `} />
            </div>
          </div>

          <div className="flex flex-col space-y-2">
            <Label htmlFor="whatsappNumber" className="pl-1">Degree</Label>
            <div className="relative ">
            <span className="absolute left-3 top-1/2 transform -translate-y-1/2"><MessageCircle size={20}></MessageCircle></span>
            <Input name="degree"  value={formData.degree||''} onChange={handleChange}  className={`w-full  border border-gray-400 rounded-md pl-10 pr-3 py-2 `} />
            </div>
          </div>
...
export default EditBoardUniversityModal;