...
import { UpdateAnnualIncome} from '@/services/student-apis';
import { AnnualIncome } from '@/types/resources/annual-income';

interface EditAnnualIncome {
    type:string;
    data:AnnualIncome;
  onClose:()=>void;
}

const EditIncomeModal: React.FC<EditAnnualIncome> = ({type, data, onClose }) => {
...
const updateData = useMutation({
  mutationFn: (formData:AnnualIncome) => {
    if (data.id !== undefined) {
      return UpdateAnnualIncome(formData, data.id);
    }
    throw new Error(`${type} ID is undefined`);
  },
  onSuccess: () => {
    queryClient.invalidateQueries({ queryKey: ["annualIncome"] });
    onClose();
  },
});
       ...
       <DialogContent className="w-[90vw] max-w-[900px] p-6  rounded-lg">
        ...
        <div className="grid grid-cols-2 gap-8 p-4">
          ...
          <div className="flex flex-col space-y-2">
            <Label htmlFor="range" className="pl-1">Range</Label>
