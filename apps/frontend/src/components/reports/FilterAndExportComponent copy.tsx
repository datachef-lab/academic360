...
import { useDownloadFilterStore } from "../globals/useDownloadFilterStore";
...
const FilterAndExportComponent: React.FC = () => {
  ...
  const { setDownloadFilters , downloadUiFilters, setDownloadUiFilters } = useDownloadFilterStore();
  ...
  const handleApplyFilters = () => {
    if (location.pathname === '/home/downloads') {
      setDownloadFilters({
        stream: downloadUiFilters.selectedStream?.name || null,
        year: downloadUiFilters.selectedYear,
        framework: downloadUiFilters.selectedFramework,
        semester: downloadUiFilters.selectedSemester,
      });
    } else {
      setFilters({
  ...
  const handleStreamSelect = (option: { name: string }) => {
    if (location.pathname === '/home/downloads') {
      setDownloadUiFilters({ selectedStream: option });
    } else {
  ...
  const handleYearSelect = (year: Year) => {
    if (location.pathname === '/home/downloads') {
      setDownloadUiFilters({ selectedYear: year });
    } else {
  ...
  const handleFrameworkSelect = (framework: Framework) => {
    if (location.pathname === '/home/downloads') {
      setDownloadUiFilters({ selectedFramework: framework });
    } else {
  ...
  const handleSemesterSelect = (semester: number) => {
    if (location.pathname === '/home/downloads') {
      setDownloadUiFilters({ selectedSemester: semester });
    } else {
       ...
       <DropdownMenu>
         <DropdownMenuTrigger asChild>
           <Button className="flex items-center gap-2 bg-white text-slate-700 hover:bg-slate-100 border border-slate-200 rounded-xl shadow-sm px-4 py-2">
             ...
             {downloadUiFilters.selectedStream? downloadUiFilters.selectedStream.name : "Stream"}
       ...
       <DropdownMenu>
         <DropdownMenuTrigger asChild>
           <Button className="flex items-center gap-2 bg-white text-slate-700 hover:bg-slate-100 border border-slate-200 rounded-xl shadow-sm px-4 py-2">
             ...
             {downloadUiFilters.selectedYear || "Year"}
             <ChevronDown className="h-4 w-4 text-slate-500" />
           </Button>
         </DropdownMenuTrigger>
       ...
       <DropdownMenu>
         <DropdownMenuTrigger asChild>
           <Button className="flex items-center gap-2 bg-white text-slate-700 hover:bg-slate-100 border border-slate-200 rounded-xl shadow-sm px-4 py-2">
             ...
             {downloadUiFilters.selectedFramework || "Framework"}
             <ChevronDown className="h-4 w-4 text-slate-500" />
           </Button>
         </DropdownMenuTrigger>
         <DropdownMenuContent className="shadow-lg rounded-xl border border-slate-200 bg-white">
       ...
       <DropdownMenu>
         <DropdownMenuTrigger asChild>
           <Button className="flex items-center gap-2 bg-white text-slate-700 hover:bg-slate-100 border border-slate-200 rounded-xl shadow-sm px-4 py-2">
             ...
             {downloadUiFilters.selectedSemester ? `Sem ${downloadUiFilters.selectedSemester}` : "Semester"}
             <ChevronDown className="h-4 w-4 text-slate-500" />
           </Button>
         </DropdownMenuTrigger>
         <DropdownMenuContent className="shadow-lg rounded-xl border border-slate-200 bg-white">
