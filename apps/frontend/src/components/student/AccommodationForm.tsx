...
const AccommodationForm = () => {
  ...
  const createData = useMutation({
    mutationFn:(formData:Accommodation) => createAccommodation(formData),
    onSuccess: (data) => {
      console.log("Data saved:", data);
    },
  ...
  useEffect(() => {
    if (data?.payload) {
      ...
      setFormData((prev) => ({
        ...prev,
        ...data.payload,
        startDate: new Date(data.payload.startDate),
        endDate: new Date(data.payload.endDate),
        addressData: {
          ...prev.addressData,
          ...addressData,
        },
      ...
      <div className="max-w-[90%] w-full grid grid-cols-1 gap-7">
        ...
        <Accordion type="multiple" className="w-full space-y-5">
          <AccordionItem value="address" className="border rounded-md shadow-sm px-8">
            <AccordionTrigger className="text-lg font-semibold ">Address</AccordionTrigger>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent className="bg-white dark:bg-gray-700 shadow-lg rounded-lg">
                    <DropdownMenuRadioGroup
                      ...
                      onValueChange={(value) => handleSelect(value, "locality")}
                    >
                      {locality.map((place) => (
                        <DropdownMenuRadioItem key={place} value={place}>
                          {place.replace("_", " ")}
