...
import { MapPin, Calendar, Edit, Clock } from "lucide-react";
import { State } from "@/types/resources/state.types";

export const stateColumns = (onEditRow?: (rowData: State) => void): ColumnDef<State>[] => [
  {
    accessorKey: "id",
    header: "ID",
    cell: ({ row }) => {
  ...
  },
  {
    ...
    header: "Country ID",
    cell: ({ row }) => {
      const countryId = row.getValue("countryId") as number;
      return (
