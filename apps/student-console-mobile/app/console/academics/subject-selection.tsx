...
import { AlertCircle, Info, Loader2 } from "lucide-react-native";
import { router } from "expo-router";
import React, { useState } from "react";
import { ActivityIndicator, Modal, Pressable, ScrollView, Text, View } from "react-native";
import SubjectSelectionInstructions from "./subject-selection-instructions";
...
export default function SubjectSelectionScreen() {
  ...
  const rawName = student?.programCourse?.course?.name ?? student?.programCourse?.name ?? "";
  const programKey = rawName
    .normalize("NFKD")
    .replace(/[^A-Za-z]/g, "")
    .toUpperCase();
  const isBlocked = blocked.has(programKey);
    ...
    return (
      <View className="flex-1 items-center justify-center p-8" style={{ backgroundColor: theme.background }}>
        ...
        <Text style={{ color: theme.text, fontSize: 16, textAlign: "center", opacity: 0.9 }}>
          ...
          {student?.programCourse?.course?.name?.trim()?.toUpperCase() ||
            student?.programCourse?.name?.trim()?.toUpperCase() ||
            "your program"}{" "}
          students.
        </Text>
      </View>
