...
import type { ExamDto, ExamCandidateDto } from "@academic/db/dtos/exams";
...
export async function downloadAdmitCard(
  examId: number,
  studentId: number,
): Promise<Blob> {
  const response = await axiosInstance.get(
