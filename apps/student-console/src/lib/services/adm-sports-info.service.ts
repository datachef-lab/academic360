...
export async function updateSportsInfo(
  id: number,
  data: Partial<Omit<SportsInfo, "id" | "createdAt" | "updatedAt">>,
) {
  try {
...
export async function deleteSportsInfo(id: number) {
  try {
    const result = await dbPostgres.delete(sportsInfo).where(eq(sportsInfo.id, id)).returning();
    return result[0];
  } catch (error) {
...
export async function getSportsInfoById(id: number) {
  try {
    const result = await dbPostgres.select().from(sportsInfo).where(eq(sportsInfo.id, id));
    return result[0];
  } catch (error) {
