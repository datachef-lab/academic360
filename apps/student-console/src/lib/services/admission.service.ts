...
} from "./adm-admision-courses.service";
...
export async function createAdmissionWithCourses(
  admission: Admission,
  courseIds: number[],
...
export async function findAdmissionById(id: number): Promise<AdmissionDto | null> {
  const [foundAdmission] = await dbPostgres.select().from(admissions).where(eq(admissions.id, id));
...
export async function findAllAdmissions(
  page: number = 1,
  size: number = 10,
  filters?: { isClosed: boolean; isArchived: boolean },
...
export async function deleteAdmission(id: number) {
  ...
  const [deletedAdmission] = await dbPostgres.delete(admissions).where(eq(admissions.id, id));
...
export async function getApplicationFormsByAdmissionId(
  ...
  const baseQuery = dbPostgres
    .select({
      ...
      name: sql<string>`${admissionGeneralInfo.firstName} || ' ' || ${admissionGeneralInfo.lastName}`.as(
        "name",
      ),
      category: categories.name,
      religion:
        sql<string>`COALESCE((SELECT ${religion.name} FROM ${admissionGeneralInfo} LEFT JOIN ${religion} ON ${admissionGeneralInfo.religionId} = ${religion.id} WHERE ${admissionGeneralInfo.applicationFormId} = ${applicationForms.id} LIMIT 1), (SELECT ${religion.name} FROM ${admissionAdditionalInfo} LEFT JOIN ${religion} ON ${admissionAdditionalInfo.religionId} = ${religion.id} WHERE ${admissionAdditionalInfo.applicationFormId} = ${applicationForms.id} LIMIT 1))`.as(
          "religion",
        ),
      ...
      course:
        sql<string>`(SELECT ${courses.name} FROM ${admissionCourseApplications} LEFT JOIN ${courses} ON ${admissionCourseApplications.admissionCourseId} = ${courses.id} WHERE ${admissionCourseApplications.applicationFormId} = ${applicationForms.id} LIMIT 1)`.as(
          "course",
        ),
      boardUniversity:
        sql<string>`(SELECT ${boardUniversities.name} FROM ${admissionAcademicInfo} LEFT JOIN ${boardUniversities} ON ${admissionAcademicInfo.boardUniversityId} = ${boardUniversities.id} WHERE ${admissionAcademicInfo.applicationFormId} = ${applicationForms.id} LIMIT 1)`.as(
          "boardUniversity",
        ),
    })
    .from(applicationForms)
    ...
    .leftJoin(
      admissionAdditionalInfo,
      eq(applicationForms.id, admissionAdditionalInfo.applicationFormId),
    )
