...
export async function getAllAnnualIncomes() {
  const incomes = await dbPostgres
    .select()
    .from(annualIncomes)
    .where(eq(annualIncomes.disabled, false));
  return incomes;
}
  ...
  try {
    ...
    if (givenAnnualIncome.range) {
      const existingIncome = await dbPostgres
        .select()
        .from(annualIncomes)
        .where(and(eq(annualIncomes.range, givenAnnualIncome.range), ne(annualIncomes.id, id)))
        .limit(1);
