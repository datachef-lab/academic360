...
export async function getAllInstitutions() {
  return await dbPostgres
    .select()
    .from(institutions)
    .where(eq(institutions.disabled, false))
...
export async function createInstitution(
  ...
  try {
    const [newInstitution] = await dbPostgres
      .insert(institutions)
      .values({ name, degreeId, addressId, sequence: sequence })
      .returning();
...
export async function updateInstitution(
  ...
  try {
    const [updatedInstitution] = await dbPostgres
      .update(institutions)
      .set({ name, degreeId, addressId, sequence: sequence, updatedAt: new Date().toISOString() })
      .where(eq(institutions.id, id))
      .returning();
...
export async function toggleInstitutionStatus(id: number): Promise<InstitutionResult> {
  try {
    ...
    const [updatedInstitution] = await dbPostgres
      .update(institutions)
      .set({ disabled: !currentInstitution.disabled, updatedAt: new Date().toISOString() })
      .where(eq(institutions.id, id))
      .returning();

    return {
      success: true,
      message: `Institution ${updatedInstitution?.disabled ? "disabled" : "enabled"} successfully.`,
      data: updatedInstitution,
    };
  } catch (error) {
