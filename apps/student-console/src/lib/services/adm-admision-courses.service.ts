...
export async function createAdmissionCourse(givenAdmCourse: AdmissionCourse) {
  ...
  const [newCourse] = await dbPostgres.insert(admissionCourses).values(givenAdmCourse).returning();
...
export async function findAllAdmissionCourses() {
  const admissionCoursesList = await dbPostgres.select().from(admissionCourses);
...
export async function deleteAdmissionCourse(id: number) {
  const deleted = await dbPostgres.delete(admissionCourses).where(eq(admissionCourses.id, id));
