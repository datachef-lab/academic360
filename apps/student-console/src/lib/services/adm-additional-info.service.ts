...
export async function createAdmissionAdditionalInfo(
  ...
  const [existingEntry] = await dbPostgres
    ...
    .where(
      and(eq(admissionAdditionalInfo.applicationFormId, givenAdditionalInfo.applicationFormId)),
    );

  if (existingEntry) {
    return {
      additionalInfo: existingEntry,
      message: "Additional info already exists for this application.",
    };
  }
