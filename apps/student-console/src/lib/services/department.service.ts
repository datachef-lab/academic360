...
export async function createDepartment(
  ...
  const [created] = await dbPostgres.insert(departments).values(data).returning();
  return created;
}
...
export async function getDepartmentById(id: number): Promise<Department | null> {
  const [found] = await dbPostgres.select().from(departments).where(eq(departments.id, id));
  return found || null;
}
...
export async function deleteDepartment(id: number): Promise<boolean> {
  const result = await dbPostgres.delete(departments).where(eq(departments.id, id));
