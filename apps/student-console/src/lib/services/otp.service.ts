...
export async function verifyOtp(
  type: (typeof otpType.enumValues)[number],
  recipient: string,
  otp: string,
) {
  ...
  const [foundOtp] = await dbPostgres
    .select()
    .from(otps)
    .where(and(eq(otps.recipient, recipient), eq(otps.type, type), eq(otps.otp, otp)))
  ...
  if (!foundOtp) {
    ...
    const allOtps = await dbPostgres
      .select()
      .from(otps)
      .where(and(eq(otps.recipient, recipient), eq(otps.type, type)))
      .orderBy(desc(otps.createdAt));
