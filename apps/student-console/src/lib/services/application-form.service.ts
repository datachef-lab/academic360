...
export async function createApplicationForm(
  ...
  if (existingEntry) {
    return {
      ...
      message: "Application form already exists for this student. Try login to continue.",
    };
  }
...
export async function findApplicationFormsByAdmissionId(admissionId: number) {
  ...
  const dtos = (await Promise.all(forms.map(async (form) => await formatAppform(form)))).filter(
    (form): form is ApplicationFormDto => form !== null,
  );
...
export async function deleteApplicationForm(id: number) {
  ...
  await dbPostgres.delete(applicationForms).where(eq(applicationForms.id, id));
