...
export async function createCourse(course: DbCourse) {
  const [existingCourse] = await dbPostgres
    .select()
    .from(courses)
    .where(ilike(courses.name, course.name.trim()));

  if (existingCourse) {
    return null;
  }

  const [newCourse] = await dbPostgres.insert(courses).values(course).returning();
...
export async function findAllDbCourses(page: number = 1, limit: number = 10) {
  ...
  const paginatedCourses = await dbPostgres.select().from(courses).limit(limit).offset(offset);

  const totalCourses = await dbPostgres.select({ count: sql<string>`count(*)` }).from(courses);
...
export async function findCourseById(id: number) {
  const [foundCourse] = await dbPostgres.select().from(courses).where(eq(courses.id, id));
