...
export async function getAllBoardUniversities(disabled?: boolean) {
  ...
  if (disabled !== undefined) {
    ...
    return await Promise.all(arr.map(async (ele) => await formatToDto(ele)));
  }
  return await Promise.all((await query).map(async (ele) => await formatToDto(ele)));
}
...
export async function createBoardUniversity(data: BoardUniversityDto) {
  ...
  const [newUniversity] = await dbPostgres.insert(boardUniversities).values(base).returning();
...
export async function updateBoardUniversity(
  id: number,
  data: Omit<BoardUniversityDto, "id" | "createdAt" | "updatedAt">,
) {
  const { subjects, ...base } = data;
...
async function formatToDto(boardUniversity: BoardUniversity): Promise<BoardUniversityDto | null> {
  ...
  if (boardUniversity.degreeId) {
    const [foundDegree] = await dbPostgres
      .select()
      .from(degree)
      .where(eq(degree.id, boardUniversity.degreeId));
