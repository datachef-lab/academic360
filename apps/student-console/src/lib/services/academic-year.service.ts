...
export async function createAcademicYear(
  ...
  const [created] = await dbPostgres.insert(academicYears).values(data).returning();
  return created;
}
...
export async function getAcademicYearById(id: number): Promise<AcademicYear | null> {
  const [found] = await dbPostgres.select().from(academicYears).where(eq(academicYears.id, id));
  return found || null;
}
...
export async function deleteAcademicYear(id: number): Promise<boolean> {
  const result = await dbPostgres.delete(academicYears).where(eq(academicYears.id, id));
