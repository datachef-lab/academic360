...
export default function TimeTableCard({ time, mockData }: TimeTableCardProps) {
  const getClassForDayAndTime = (day: (typeof weekDays)[number], time: string) => {
  ...
  return (
    <tr className="border-b border-gray-200 hover:bg-gray-50 transition-colors duration-150 odd:bg-white even:bg-gray-50">
      ...
      {weekDays.map((day) => {
        ...
        return (
          <td key={day} className="p-3 border-b align-top min-w-[120px] text-sm space-y-1">
            {cls ? (
              <div>
                <div className="font-semibold text-indigo-800 text-base">{cls.subject}</div>
