...
export default function TimeTableDialog({ open, setOpen, mockData }: TimeTableDialogProps) {
      ...
      <DialogContent className="max-w-4xl">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full border border-gray-200 rounded-lg text-center shadow-sm">
            <thead>
              <tr>
                <th className="p-3 text-left border-b bg-gray-100 text-gray-700">Time</th>
                {weekDays.map((day) => (
                  <th key={day} className="p-3 border-b bg-gray-100 text-gray-700">
                    {day}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {allTimeSlots.map((time, index) => (
                <TimeTableCard key={`time-${index}`} time={time} mockData={mockData} />
              ))}
            </tbody>
          </table>
        </div>
