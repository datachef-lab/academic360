...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { success: false, message: "Failed to fetch nationalities", error: error.message },
      { status: 500 },
    );
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    const [existingNationality] = await dbPostgres
      .select()
      .from(nationality)
      .where(ilike(nationality.name, validatedData.name.trim()));

    if (existingNationality) {
      return NextResponse.json({ success: true, data: existingNationality });
    }
    ...
    return NextResponse.json(
      { success: false, message: "Failed to create nationality", error: error.message },
      { status: 400 },
    );
  }
}
...
export async function PUT(req: Request) {
  try {
    ...
    return NextResponse.json(
      { success: false, message: "Failed to update nationality", error: error.message },
      { status: 400 },
    );
  }
}
...
export async function DELETE(req: Request) {
  try {
    ...
    return NextResponse.json(
      { success: false, message: "Failed to delete nationality", error: error.message },
      { status: 500 },
    );
  }
}
