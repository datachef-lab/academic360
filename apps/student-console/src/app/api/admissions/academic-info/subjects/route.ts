...
export async function GET(req: NextRequest) {
  try {
    ...
    if (id) {
      ...
      if (!subject) {
        return NextResponse.json({ message: "Student subject not found" }, { status: 404 });
      }
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ message: "Student subject ID is required" }, { status: 400 });
    }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ message: "Student subject deleted successfully" }, { status: 200 });
  } catch (error) {
