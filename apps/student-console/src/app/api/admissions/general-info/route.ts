...
export async function GET(req: NextRequest) {
  try {
    ...
    if (id) {
      ...
      if (!generalInfo) {
        return NextResponse.json({ message: "General info not found" }, { status: 404 });
      }
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ message: "General info ID is required" }, { status: 400 });
    }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ message: "General info deleted successfully" }, { status: 200 });
  } catch (error) {
