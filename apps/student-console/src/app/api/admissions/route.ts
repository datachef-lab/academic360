...
import {
  createAdmission,
  createAdmissionWithCourses,
  findAllAdmissionSummary,
  admissionStats,
  updateAdmission,
...
export async function PUT(request: Request) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: "Admission ID is required for update" }, { status: 400 });
    }
    const lastDate = endDate || body.lastDate;
    ...
    await dbPostgres.transaction(async (tx) => {
      // Update admission
      await tx.update(admissions).set(updateData).where(eq(admissions.id, id));
      ...
      if (Array.isArray(courses)) {
        for (const c of courses) {
          ...
          if (existing) {
            ...
            await tx.insert(admissionCourses).values({
              admissionId: id,
              courseId: c.courseId,
              disabled: c.disabled,
