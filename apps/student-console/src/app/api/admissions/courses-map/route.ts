...
export async function POST(req: NextRequest) {
  try {
    ...
    if (result.message === "Admission course already exists.") {
      return NextResponse.json({ message: result.message, course: result.course }, { status: 409 });
    }
  ...
  try {
    ...
    if (id) {
      if (withDetails === "true") {
        ...
        if (!course) {
          return NextResponse.json({ message: "Admission course not found" }, { status: 404 });
        }
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ message: "Admission course ID is required" }, { status: 400 });
    }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ message: "Admission course deleted successfully" }, { status: 200 });
  } catch (error) {
