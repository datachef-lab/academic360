...
export function FeesDueAlertDialog({
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      {/* Single clean column: notification → dues summary → declaration panel → footer bar.
          (The big side illustration made the dialog feel cluttered and was dropped.) */}
      <AlertDialogContent className="max-w-xl gap-0 overflow-hidden p-0">
        <div className="max-h-[80vh] overflow-y-auto px-6 pb-5 pt-6 sm:px-7">
          <AlertDialogHeader className="space-y-3 text-left sm:text-left">
            ...
            <AlertDialogDescription className="space-y-2.5 text-sm leading-relaxed text-gray-600">
              <span className="block">
                As per our records, your Semester{" "}
                <span className="font-semibold text-gray-800">{semesterDisplay}</span> enrolment fee
                is not paid despite multiple reminders sent to your Institutional Email ID and/or
                registered mobile number. Consequently, you are currently not considered a bonafide
                student of the College.
              </span>
              <span className="block">
                Please note that payment of the Semester{" "}
                <span className="font-semibold text-gray-800">{semesterDisplay}</span> enrolment fee
                is mandatory to maintain your bonafide student status and to become eligible to
          ...
          {visibleFees.length > 0 && (
            <div className="mt-4 overflow-hidden rounded-lg border border-gray-200">
              <div className="max-h-32 divide-y divide-gray-100 overflow-y-auto">
                {visibleFees.map((fee) => {
          ...
          <div className="mt-4 space-y-3 rounded-lg border border-indigo-100 bg-indigo-50/40 p-4">
            ...
            <div className="flex items-start gap-3 text-sm leading-relaxed text-gray-700">
              ...
              <span>
                <label htmlFor="fee-due-ack" className="cursor-pointer">
                  ...
                  and undertake to clear the dues by
                </label>{" "}
                <input
                  type="date"
                  value={clearByDate}
        ...
        <AlertDialogFooter className="gap-2 border-t border-gray-200 bg-gray-50 px-6 py-4 sm:justify-between">
          <Button
            variant="outline"
            className="mt-0"
            onClick={() => {
