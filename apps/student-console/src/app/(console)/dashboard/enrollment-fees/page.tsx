...
export default function FeesPage() {
  ...
  useEffect(() => {
    const fetchInstalments = async () => {
      try {
        ...
        const summaries: Record<string, { total: number; paid: number; status: string }> = {};
  ...
  const getStatusInfo = (instalment: Instalment) => {
    ...
    const lastDate = instalment.metadata.lastDate ? new Date(instalment.metadata.lastDate) : null;
        ...
        <div className="space-y-8">
          {selectedFeeType !== null ? (
                                  ...
                                  <Table>
                                    ...
                                    <TableBody>
                                      {instalments.map((instalment, index) => {
                                        const statusInfo = getStatusInfo(instalment);
                                        return (
                                          <TableRow
                                              ...
                                              {instalment.hasPaid ? (
                                                <div className="space-y-1">
                                                  <span className="text-green-600 font-medium flex items-center gap-1">
                                                    ...
                                                    Paid on {formatDate(instalment.paidAt)}
                                                  </span>
                                                  <span className="text-xs text-gray-500 flex items-center gap-1">
                                                    <CreditCard className="h-3 w-3" />
                                                    {instalment.details.paymentMode} | Challan:{" "}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {Object.entries(groupedInstalments).map(([feeType, feeInstalments]) => {
                              ...
                              <div className="text-xs text-gray-700 space-y-1.5">
                                ...
                                {firstInstalment.fg && (
                                  <p className="text-green-700">
                                    <span className="font-medium">Fine Grace:</span> Applied
                                  </p>
                                )}
                              </div>
                            </div>
