...
export function SportsCategoryDialog({
  onSuccess,
  children,
  sportsCategory,
}: SportsCategoryDialogProps) {
      ...
      if (result && result.success) {
        ...
        toast({
          ...
          description:
            result.error ||
            (sportsCategory
              ? "Failed to update sports category."
              : "Failed to add sports category."),
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>
            {sportsCategory ? "Edit Sports Category" : "Add New Sports Category"}
          </DialogTitle>
          <DialogDescription>
            {sportsCategory
              ? "Update the details for the sports category."
              : "Enter the details for the new sports category."}
          </DialogDescription>
        </DialogHeader>
