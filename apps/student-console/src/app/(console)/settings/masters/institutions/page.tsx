...
function StatusBadge({ status }: { status: string }) {
  return (
    <span
      className={`px-2 py-1 rounded text-xs font-semibold ${status === "Active" ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-600"}`}
    >
      {status}
    </span>
  );
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            >
              ...
              {degrees.map((deg) => (
                <option key={deg.id} value={String(deg.id)}>
                  {deg.name}
                </option>
              ))}
            </select>
          ...
          <div>
            ...
            <Input
              type="number"
              value={
                form.sequence !== undefined && form.sequence !== null ? String(form.sequence) : ""
              }
  ...
  const handleSave = async (data: InstitutionForm) => {
    if (editData) {
      ...
      if (id !== undefined) {
        ...
        setInstitutions((insts: InstitutionForm[]) =>
          insts.map((i: InstitutionForm) => (i.id === updated.id ? updated : i)),
        );
      }
    } else {
