      ...
      if (result && result.success) {
        toast({
          ...
          description:
            result.message ||
            (annualIncome
              ? "Annual income range updated successfully."
              : "Annual income range added successfully."),
        });
        setIsOpen(false);
        setIncomeRange("");
        onSuccess();
        ...
        toast({
          ...
          description:
            result.error ||
            (annualIncome
              ? "Failed to update annual income range."
              : "Failed to add annual income range."),
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>
            {annualIncome ? "Edit Annual Income Range" : "Add New Annual Income Range"}
          </DialogTitle>
          <DialogDescription>
            {annualIncome
              ? "Update the details for the annual income range."
