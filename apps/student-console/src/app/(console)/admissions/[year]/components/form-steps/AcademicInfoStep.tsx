...
export default function AcademicInfoStep({
  ...
  // Local aliases to relax typings for shadcn components in this file
  const SelectTriggerFixed = SelectTrigger as unknown as React.ComponentType<any>;
  const SelectContentFixed = SelectContent as unknown as React.ComponentType<any>;
  const SelectItemFixed = SelectItem as unknown as React.ComponentType<any>;
  const DialogTriggerFixed = DialogTrigger as unknown as React.ComponentType<any>;
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (academicSubjectsResponse.ok) {
          ...
          console.error(
            "Failed to fetch academic subjects:",
            academicSubjectsResponse.status,
            academicSubjectsResponse.statusText,
          );
          setAcademicSubjects([]);
        }
        ...
        if (applicationForm.id) {
          try {
            const existingAcademicInfoResponse = await fetch(
              `/api/admissions/academic-info?applicationFormId=${applicationForm.id}`,
            );
  ...
  const handleSaveInstituteDetails = (details: InstituteDetails) => {
    ...
    setAcademicInfo((prev) => ({
      ...
      otherPreviouslyRegisteredCourse:
        details.previouslyRegisteredCourse === "Other Course" ? details.otherCourse : null,
    }));
  };
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      const {
        createdAt: createdAtDraft,
        updatedAt: updatedAtDraft,
        ...academicInfoDataDraft
      } = {
        ...academicInfo,
        applicationFormId: applicationForm.id,
      };
        ...
        <>
          <div className="mb-4">
            <h2 className="text-base sm:text-lg font-semibold mb-2">
              Step 2 of 5 - Academic Details (Sr. No. 14 to 17)
            </h2>
          ...
          <div className="grid grid-cols-1 gap-4 sm:gap-6">
            <div>
              <label className="flex items-center mb-1 text-sm">
                14. Board Result Status <span className="text-red-600">*</span>
              </label>
              <Select
            ...
            <div>
              ...
              >
                ...
                <SelectContentFixed>
                  <SelectItemFixed value="0">Select Board/University</SelectItemFixed>
                  {boardUniversities.map(
                    (board) =>
                      board.id && (
                        <SelectItemFixed key={board.id} value={board.id.toString()}>
                          {board.name}
                        </SelectItemFixed>
                      ),
                  )}
            ...
            <div>
              <label className="flex items-center mb-1 text-sm">
                16. Subjectwise Marks <span className="text-red-600">*</span>
              </label>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th
                          scope="col"
                          className="px-2 sm:px-4 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-32 sm:w-48"
                        >
                          Subject
                        </th>
                        <th
                        ...
                        <th
                          scope="col"
                          className="px-2 sm:px-4 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-32 sm:w-40"
                        >
                          Status
                        </th>
                      </tr>
            ...
            <div>
              <label className="flex items-center mb-1 text-sm">
                17. Institute Details <span className="text-red-600">*</span>
              </label>
              ...
              {enteredInstituteDetails && (
                <div className="mt-4 p-3 sm:p-4 border rounded-md bg-gray-50">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs sm:text-sm">
                    ...
                    <p>
                      <strong>School No:</strong> {enteredInstituteDetails.schoolNo}
                    </p>
                    <p>
                      <strong>Center No:</strong> {enteredInstituteDetails.centerNo}
                    </p>
                    <p>
                      <strong>Admit Card ID:</strong> {enteredInstituteDetails.admitCardId}
                    </p>
                    <p>
                      <strong>Institute:</strong> {enteredInstituteDetails.institute}
                    </p>
                    {enteredInstituteDetails.institute === "Other Institute" && (
                      <p>
                        <strong>Other Institute:</strong> {enteredInstituteDetails.otherInstitute}
                      </p>
                    )}
                    <p>
                      <strong>Medium:</strong> {enteredInstituteDetails.medium}
                    </p>
                    <p>
                      <strong>Year Of Passing:</strong> {enteredInstituteDetails.yearOfPassing}
                    </p>
                    <p>
                      <strong>Stream:</strong> {enteredInstituteDetails.stream}
                    </p>
                    <p>
                      <strong>Calcutta University Registered:</strong>{" "}
                      {enteredInstituteDetails.calcuttaUniversityRegistered}
                    </p>
                    {enteredInstituteDetails.calcuttaUniversityRegistered === "Yes" && (
                      <p>
                        <strong>Calcutta University Registration No:</strong>{" "}
                        {enteredInstituteDetails.calcuttaUniversityRegistrationNo}
                      </p>
                    )}
                    <p>
                      <strong>Previously Registered Course:</strong>{" "}
                      {enteredInstituteDetails.previouslyRegisteredCourse}
                    </p>
                    {enteredInstituteDetails.previouslyRegisteredCourse === "Other Course" && (
                      <p>
                        <strong>Other Course:</strong> {enteredInstituteDetails.otherCourse}
                      </p>
                    )}
                    <p>
                      <strong>Previous College:</strong> {enteredInstituteDetails.previousCollege}
                    </p>
                    {enteredInstituteDetails.previousCollege === "Other college" && (
                      <p>
                        <strong>Other college:</strong> {enteredInstituteDetails.otherCollege}
                      </p>
                    )}
                  </div>
                </div>
