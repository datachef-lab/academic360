...
interface Admission {
  ...
  isClosed: boolean;
  // Add any other properties your admission object has
}
...
export default function AdmissionsClient({
  ...
  const accentBar = (
    <span className="inline-block w-2 h-6 rounded bg-gradient-to-b from-purple-500 to-pink-400 mr-3 align-middle" />
  );
  ...
  const gradientText =
    "bg-gradient-to-r from-purple-700 via-blue-600 to-pink-500 bg-clip-text text-transparent";
  ...
  if (error || !admission) {
    return (
      <div
        className={"min-h-screen flex flex-col items-center justify-center relative " + fontClass}
      >
        {animatedBg}
        {floatingShapes}
        <Image
          src="/admission-close-besc.png"
          alt="Admissions Closed"
          width={120}
          height={120}
          className="mx-auto mb-6 drop-shadow-xl z-10"
        />
        <h1 className="text-3xl md:text-4xl font-extrabold text-gray-800 mb-2 z-10">
          Admissions Not Open
        </h1>
        <p className="text-gray-600 mb-4 z-10">
          Admissions for {currentYear} are currently closed or not available.
        </p>
        ...
        <Button
          asChild
          className="mt-2 bg-gradient-to-r from-purple-600 to-blue-600 text-white font-semibold px-6 py-2 rounded-full shadow-lg hover:scale-105 hover:shadow-xl transition-transform z-10"
        >
  ...
  return (
    <div
      className={"max-h-screen flex flex-col relative min-h-screen " + fontClass}
      style={{ height: "100vh" }}
    >
      {animatedBg}
      {floatingShapes}
      ...
      <main className="flex-1 flex flex-col items-center justify-center w-full px-2 md:px-0">
        ...
        <section
          className="flex flex-col items-center justify-center pt-6 pb-4 relative z-10 w-full"
          style={{ minHeight: 0 }}
        >
          <Image
            src="/besc-logo.jpeg"
            ...
            width={64}
            height={64}
            className="rounded-full border-2 border-white shadow-lg mb-3 bg-white z-10"
          />
          <h1
            className={
              "text-2xl md:text-3xl font-extrabold text-center tracking-tight mb-1 " + gradientText
            }
          >
            {isClosed ? "Admissions Closed" : `Admissions Open for ${currentYear}`}
          </h1>
          ...
          {!isClosed && (
            <Button
              asChild
              size="sm"
              className="mt-1 bg-gradient-to-r from-purple-600 to-blue-600 text-white font-bold px-6 py-2 rounded-full shadow text-base hover:scale-105 hover:shadow-lg transition-transform"
            >
              <Link href={`/admissions/${currentYear}`}>Apply Now</Link>
            </Button>
          )}
        </section>
          ...
          <section className="w-full max-w-xl mx-auto mt-4 px-1 md:px-0 relative z-10">
            ...
            <div className="overflow-x-auto rounded-xl shadow bg-white/80 backdrop-blur border border-white/40 max-h-48 md:max-h-56 overflow-y-auto">
              <table className="min-w-full text-xs md:text-sm rounded-xl overflow-hidden">
                ...
                <tbody>
                  {courses.filter(
                    (c: any) => !c.disabled && !c.isClosed && c.course && !c.course.disabled,
                  ).length === 0 ? (
                    <tr>
                      <td colSpan={2} className="py-3 text-center text-gray-500">
                        No courses available for this admission.
                      </td>
                    </tr>
                  ) : (
                    courses
                      ...
                      .map((c: any, idx: number) => (
                        <tr
                          key={c.id}
                          className={
                            "transition hover:bg-purple-50/40 " +
                            (idx % 2 === 0 ? "bg-white/60" : "bg-white/40")
                          }
                        >
                          <td className="py-2 px-3 text-gray-900 whitespace-nowrap font-semibold">
                            {idx + 1}
                          </td>
                          <td className="py-2 px-3 text-gray-800 whitespace-nowrap font-medium">
                            {c.course?.name || "Unnamed Course"}
                          </td>
                        </tr>
                      ))
                  )}
        ...
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-between gap-2 px-4 text-xs text-gray-500">
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-2 ml-2">
              <a href="#" className="text-purple-400 hover:text-purple-700" aria-label="Facebook">
                <svg fill="currentColor" viewBox="0 0 24 24" className="w-3 h-3">
                  <path d="M22.675 0h-21.35C.595 0 0 .592 0 1.326v21.348C0 23.406.595 24 1.325 24h11.495v-9.294H9.692v-3.622h3.128V8.413c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.797.143v3.24l-1.918.001c-1.504 0-1.797.715-1.797 1.763v2.313h3.587l-.467 3.622h-3.12V24h6.116C23.406 24 24 23.406 24 22.674V1.326C24 .592 23.406 0 22.675 0" />
                </svg>
              </a>
              <a href="#" className="text-purple-400 hover:text-purple-700" aria-label="Twitter">
                <svg fill="currentColor" viewBox="0 0 24 24" className="w-3 h-3">
              ...
              <a href="#" className="text-purple-400 hover:text-purple-700" aria-label="LinkedIn">
                <svg fill="currentColor" viewBox="0 0 24 24" className="w-3 h-3">
          ...
          <div className="flex items-center gap-1">
            <Image
              src="/datachef-logo.svg"
              alt="DataChef"
              width={16}
              height={16}
              className="rounded-full border border-purple-200 bg-white"
            />
