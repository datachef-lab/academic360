...
export class BoardUniversityService {
  static async getAllBoardUniversities(): Promise<ApiResponse<BoardUniversity[]>> {
    try {
      ...
      return {
        success: false,
        message: "Failed to fetch board universities",
        error: error.message,
      };
    }
  }

  static async createBoardUniversity(
    boardUniversity: Omit<BoardUniversity, "id" | "createdAt" | "updatedAt">,
  ): Promise<ApiResponse<BoardUniversity>> {
    try {
      const response = await fetch("/api/board-universities", {
        method: "POST",
        headers: {
  ...
  static async updateBoardUniversity(
