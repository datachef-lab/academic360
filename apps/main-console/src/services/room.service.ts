...
import type { RoomT } from "@academic/db/schemas/models/exams";
...
export async function getRoomById(id: number): Promise<ApiResponse<RoomT>> {
  try {
    const response = await axiosInstance.get<ApiResponse<RoomT>>(
      `${BASE_URL}/${id}`,
    );
    return response.data;
  } catch {
...
export async function createRoom(
  payload: Partial<RoomT>,
): Promise<ApiResponse<RoomT>> {
  try {
    const response = await axiosInstance.post<ApiResponse<RoomT>>(
      BASE_URL,
      payload,
    );
    return response.data;
...
export async function updateRoom(
  ...
  try {
    const response = await axiosInstance.put<ApiResponse<RoomT>>(
      `${BASE_URL}/${id}`,
      payload,
    );
    return response.data;
