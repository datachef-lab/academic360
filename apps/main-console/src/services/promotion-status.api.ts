...
export async function getPromotionStatusById(
  id: number,
): Promise<PromotionStatusT | null> {
  const res = await axiosInstance.get<ApiResponse<PromotionStatusT>>(
    `${BASE}/${id}`,
  );
...
export async function createPromotionStatusApi(
  ...
  const res = await axiosInstance.post<ApiResponse<PromotionStatusT>>(
    BASE,
    data,
  );
  return res.data.payload;
...
export async function updatePromotionStatusApi(
  ...
  const res = await axiosInstance.put<ApiResponse<PromotionStatusT>>(
    `${BASE}/${id}`,
    data,
  );
  return res.data.payload;
