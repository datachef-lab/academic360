...
export async function getStudentCuCorrectionRequests(studentId: number) {
  console.info(
    `[CU-REG MAIN-CONSOLE] Fetching correction requests for student: ${studentId}`,
  );
  try {
    ...
    console.info(
      `[CU-REG MAIN-CONSOLE] Correction requests response:`,
      res.data,
    );
    const p =
      "requests" in res.data.payload
        ? res.data.payload.requests
        : res.data.payload;
    const result = (
    ...
    console.error(
      `[CU-REG MAIN-CONSOLE] Error fetching correction requests:`,
      error,
    );
    throw error;
  }
...
export async function getCuCorrectionRequestById(id: number) {
  console.info(
    `[CU-REG MAIN-CONSOLE] Fetching correction request by ID: ${id}`,
  );
  try {
    const res = await axiosInstance.get<{
      success: boolean;
    ...
    console.info(
      `[CU-REG MAIN-CONSOLE] Correction request response:`,
      res.data,
    );
    return res.data.payload;
  } catch (error) {
    console.error(
      `[CU-REG MAIN-CONSOLE] Error fetching correction request:`,
      error,
    );
    throw error;
  }
