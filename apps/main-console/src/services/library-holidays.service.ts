...
): Promise<ApiResponse<LibraryHolidayRow>> {
  const res = await axiosInstance.post<ApiResponse<LibraryHolidayRow>>(BASE, body);
  return res.data;
}

export async function updateLibraryHoliday(
  id: number,
  body: Partial<LibraryHolidayUpsertBody>,
): Promise<ApiResponse<LibraryHolidayRow>> {
  const res = await axiosInstance.put<ApiResponse<LibraryHolidayRow>>(`${BASE}/${id}`, body);
  return res.data;
}
