...
export type UpdateRelatedSubjectMainInput =
  Partial<CreateRelatedSubjectMainInput> & {
    relatedSubjectSubs?: { boardSubjectName: { id: number } }[];
  };
...
export const subjectSelectionApi = {
  ...
  async listRelatedSubjectMains() {
    const res = await api.get<
      ApiResponse<
        RelatedSubjectMainDto[] | { content: RelatedSubjectMainDto[] }
      >
    >(`${BASE_MAIN}`);
  ...
  async listRelatedSubjectMainsPaginated(params: {
    ...
    const res = await api.get<
      ApiResponse<PaginatedResponse<RelatedSubjectMainDto>>
    >(`${BASE_MAIN}`, { params });
    return res.data.payload;
  },
  async getRelatedSubjectMain(id: number) {
    const res = await api.get<ApiResponse<RelatedSubjectMainDto>>(
      `${BASE_MAIN}/${id}`,
    );
    return res.data.payload;
  },
  async createRelatedSubjectMain(payload: CreateRelatedSubjectMainInput) {
    const res = await api.post<ApiResponse<RelatedSubjectMainDto>>(
      `${BASE_MAIN}`,
      payload,
    );
    return res.data.payload;
  },
  async updateRelatedSubjectMain(
    id: number,
    payload: UpdateRelatedSubjectMainInput,
  ) {
    const res = await api.put<ApiResponse<RelatedSubjectMainDto>>(
      `${BASE_MAIN}/${id}`,
      payload,
    );
    return res.data.payload;
  ...
  async listRelatedSubjectSubsByMain(mainId: number) {
    const res = await api.get<ApiResponse<RelatedSubjectSubDto[]>>(
      `${BASE_SUB}/main/${mainId}`,
    );
    return res.data.payload;
  },
  async createRelatedSubjectSub(payload: CreateRelatedSubjectSubInput) {
    const res = await api.post<ApiResponse<RelatedSubjectSubDto>>(
      `${BASE_SUB}`,
      payload,
    );
    return res.data.payload;
