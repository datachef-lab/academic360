...
import { updateAddress, getDistrictsByState as getDistrictsByStateService } from "@/services/address.service";
import type { DistrictOption } from "@/services/address.service";
import type { ReligionT as UiReligion, CategoryT as UiCategory, NationalityT as UiNationality } from "@repo/db/schemas";
import { PersonalDetailsDto, AddressDto as Address } from "@repo/db/dtos";
...
const getAddressRelationName = (
  addr: Address | null | undefined,
  key: "country" | "state" | "city" | "district" | "postOffice" | "policeStation",
): string => {
  if (!addr) return "";
  const rel = (addr as unknown as Record<string, { name?: string } | null | undefined>)[key];
  return rel?.name ?? "";
};
...
const defaultPersonalDetails: PersonalDetailsDto = {
  id: 0,
...
interface PersonalDetailProps {
  studentId: number;
  initialData?: Partial<PersonalDetailsDto> | null;
}

export default function PersonalDetail({ studentId, initialData }: PersonalDetailProps) {
  const [showSuccess, setShowSuccess] = useState(false);
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [religions, setReligions] = useState<UiReligion[]>([]);
  const [categories, setCategories] = useState<UiCategory[]>([]);
  const [nationalities, setNationalities] = useState<UiNationality[]>([]);
  const [districtOptions, setDistrictOptions] = useState<DistrictOption[]>([]);
  const initialMapped: Partial<PersonalDetailsDto> = (() => {
    const data: Partial<PersonalDetailsDto> = initialData ?? {};
    const mapSimple = (obj?: { id?: number | null; name?: string | null } | null) =>
      obj && obj.id
        ? ({ id: obj.id, name: obj.name ?? "" } as unknown as UiReligion | UiCategory | UiNationality)
        : null;
    const mapAddress = (obj?: Record<string, unknown> | null): Address | null =>
      obj
        ? ({
            id: (obj as { id?: number }).id ?? 0,
            addressLine: (obj as { addressLine?: string | null }).addressLine ?? "",
            pincode: (obj as { pincode?: string | null }).pincode ?? "",
            landmark: (obj as { landmark?: string | null }).landmark ?? "",
            phone: (obj as { phone?: string | null }).phone ?? "",
            localityType: (obj as { localityType?: LocalityType | null }).localityType ?? ("URBAN" as LocalityType),
            country: null,
            state: null,
            city: null,
            district: null,
    ...
    return {
      aadhaarCardNumber: data.aadhaarCardNumber ?? null,
      email:
        (data as unknown as { personalEmail?: string | null; email?: string | null }).personalEmail ??
        (data as unknown as { email?: string | null }).email ??
        null,
      alternativeEmail: (data as unknown as { alternativeEmail?: string | null }).alternativeEmail ?? null,
      dateOfBirth: parseDate((data as unknown as { dateOfBirth?: string | Date | null }).dateOfBirth),
      nationality: mapSimple(
        (data as unknown as { nationality?: { id?: number; name?: string } | null }).nationality ?? null,
      ) as unknown as UiNationality | null,
      religion: mapSimple(
        (data as unknown as { religion?: { id?: number; name?: string } | null }).religion ?? null,
      ) as unknown as UiReligion | null,
      category: mapSimple(
        (data as unknown as { category?: { id?: number; name?: string } | null }).category ?? null,
      ) as unknown as UiCategory | null,
      residentialAddress: mapAddress(
        (data as unknown as { residentialAddress?: Record<string, unknown> | null }).residentialAddress ?? null,
      ),
      mailingAddress: mapAddress(
        (data as unknown as { mailingAddress?: Record<string, unknown> | null }).mailingAddress ?? null,
      ),
      gender: (data as unknown as { gender?: PersonalDetailsDto["gender"] | null }).gender ?? null,
      disability: (data as unknown as { disability?: PersonalDetailsDto["disability"] | null }).disability ?? "OTHER",
    };
  })();

  const [formData, setFormData] = useState<PersonalDetailsDto>({
    ...defaultPersonalDetails,
    ...initialMapped,
    id: studentId,
    studentId: studentId,
    email: initialMapped.email ?? null,
    alternativeEmail: initialMapped.alternativeEmail ?? null,
  });

  // If profile initialData arrives later, hydrate the form immediately
  useEffect(() => {
    const mapped: Partial<PersonalDetailsDto> = (() => {
      const data: Partial<PersonalDetailsDto> = initialData ?? {};
      const mapSimple = (obj?: { id?: number | null; name?: string | null } | null) =>
        obj && obj.id
          ? ({ id: obj.id, name: obj.name ?? "" } as unknown as UiReligion | UiCategory | UiNationality)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setReligions((religionsData as unknown as UiReligion[]) ?? []);
        setCategories((categoriesData as unknown as UiCategory[]) ?? []);
        setNationalities((nationalitiesData as unknown as UiNationality[]) ?? []);
      } catch (error) {
  ...
  // Load districts when residential state changes
  useEffect(() => {
    const stateId = (formData.residentialAddress as unknown as { stateId?: number | null })?.stateId ?? null;
    if (stateId) {
      getDistrictsByStateService(stateId)
        .then((list) => setDistrictOptions(list))
        .catch(() => setDistrictOptions([]));
    } else {
      setDistrictOptions([]);
    }
  }, [formData.residentialAddress?.stateId]);
  ...
  const {
    ...
    enabled: !initialData && studentId > 0,
    retry: 1,
  ...
  useEffect(() => {
    if (personalDetails) {
      ...
      setFormData(personalDetails as PersonalDetailsDto);
    }
  }, [personalDetails]);

  const updateMutation = useMutation({
    mutationFn: async (formData: PersonalDetailsDto) => {
            ...
            <div key={name} className="flex flex-col gap-2">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  name={name}
                  type={type}
                  value={(formData[name as keyof PersonalDetailsDto] as string) || ""}
                  placeholder={label}
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFormData((prev) => ({ ...prev, gender: value as Gender }) as PersonalDetailsDto)
                }
              >
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFormData((prev) => ({ ...prev, disability: value as Disability }) as PersonalDetailsDto)
                }
              >
                <SelectTrigger className="w-full pl-10 pr-3 rounded-lg py-2">
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                <div className="flex flex-col gap-2">
                  <Label>Country</Label>
                  <Input
                    value={getAddressRelationName(formData.residentialAddress, "country")}
                    disabled
                    className="w-full"
                  />
                </div>
                <div className="flex flex-col gap-2">
                  <Label>State</Label>
                  <Input
                    value={getAddressRelationName(formData.residentialAddress, "state")}
                    disabled
                    className="w-full"
                  />
                </div>
                <div className="flex flex-col gap-2">
                  <Label>City</Label>
                  <Input
                    value={getAddressRelationName(formData.residentialAddress, "city")}
                    disabled
                    className="w-full"
                  />
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                <div className="flex flex-col gap-2">
                  ...
                  {districtOptions.length > 0 ? (
                    <Select
                      value={
                        (formData.residentialAddress?.district as unknown as { id?: number })?.id?.toString() || ""
                      }
                      onValueChange={(value) => {
                        const selected = districtOptions.find((d) => d.id.toString() === value) || null;
                        setFormData(
                          (prev) =>
                            ({
                              ...prev,
                              residentialAddress: {
                                ...prev.residentialAddress,
                                // store minimal district info for display; backend update uses IDs
                                district: selected
                                  ? ({ id: selected.id, name: selected.name } as unknown as Address["district"])
                                  : null,
                              },
                            }) as PersonalDetailsDto,
                        );
                    ...
                    <Input
                      value={getAddressRelationName(formData.residentialAddress, "district")}
                      disabled
                      className="w-full"
                    />
                  )}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Input
                    value={getAddressRelationName(formData.residentialAddress, "postOffice")}
                    disabled
                    className="w-full"
                  />
                </div>
                <div className="flex flex-col gap-2">
                  ...
                  <Input
                    value={getAddressRelationName(formData.residentialAddress, "policeStation")}
                    disabled
                    className="w-full"
                  />
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="flex flex-col gap-2">
                    <Label>Country</Label>
                    <Input
                      value={getAddressRelationName(formData.mailingAddress, "country")}
                      disabled
                      className="w-full"
                    />
                  </div>
                  <div className="flex flex-col gap-2">
                    <Label>State</Label>
                    <Input
                      value={getAddressRelationName(formData.mailingAddress, "state")}
                      disabled
                      className="w-full"
                    />
                  </div>
                  <div className="flex flex-col gap-2">
                    <Label>City</Label>
                    <Input
                      value={getAddressRelationName(formData.mailingAddress, "city")}
                      disabled
                      className="w-full"
                    />
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="flex flex-col gap-2">
                    <Label>District</Label>
                    <Input
                      value={getAddressRelationName(formData.mailingAddress, "district")}
                      disabled
                      className="w-full"
                    />
                  </div>
                  <div className="flex flex-col gap-2">
                    <Label>Post Office</Label>
                    <Input
                      value={getAddressRelationName(formData.mailingAddress, "postOffice")}
                      disabled
                      className="w-full"
                    />
                  </div>
                  <div className="flex flex-col gap-2">
                    <Label>Police Station</Label>
                    <Input
                      value={getAddressRelationName(formData.mailingAddress, "policeStation")}
                      disabled
                      className="w-full"
                    />
                  </div>
