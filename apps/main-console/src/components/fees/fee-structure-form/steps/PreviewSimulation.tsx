...
interface PreviewSimulationProps {
  feesStructure:
    | (FeesStructureDto & { feesReceiptTypeId?: number | null })
    | CreateFeesStructureDto;
...
export const PreviewSimulation: React.FC<PreviewSimulationProps> = ({
  ...
  const MIN_ROWS = 8;
  const [selectedSlabId, setSelectedSlabId] = useState<number | null>(
    feesSlabMappings[0]?.feesSlabId ?? null,
  );

  const getFeeHeadName = (id: number) => feeHeads.find((h) => h.id === id)?.name || "N/A";
  const getSlabName = (id: number) => slabs.find((s) => s.id === id)?.name || "N/A";
  ...
  const calculatedFees = useMemo((): (FeesComponent & {
    feeHeadName: string;
    ...
    const selectedSlabData = feesSlabMappings.find((slab) => slab.feesSlabId === selectedSlabId);
    ...
    return components.map((component) => ({
      ...
      adjustedAmount: component.isConcessionApplicable
        ? component.baseAmount - (component.baseAmount * selectedSlabData.feeConcessionRate) / 100
  ...
  const adjustedTotalAmount = useMemo(
    () => calculatedFees.reduce((sum, fee) => sum + (fee.adjustedAmount ?? fee.baseAmount), 0),
    [calculatedFees],
  );
      ...
      <div className="lg:col-span-1 h-full">
        <div className="bg-white rounded-lg border border-gray-200 h-full flex flex-col">
          <div className="px-4 py-3 bg-gray-50 border-b border-gray-200">
            <h4 className="text-sm font-medium text-gray-900">Details Summary</h4>
          </div>
            ...
            <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900">
                    {feesStructure.academicYear?.year
                      ? `${new Date(feesStructure.academicYear.year).getFullYear()}`
                      : "N/A"}
                  </p>
                </div>
              </div>
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900">
                    {"feesStructure.course?.name"} (Semester {feesStructure.class.name})
                  </p>
                </div>
              </div>
              <div className="flex items-start space-x-3">
              ...
              {feesStructure.advanceForCourse && (
                <div className="flex items-start space-x-3">
                  <Clock className="w-5 h-5 text-gray-400 mt-0.5" />
                  <div>
                    <p className="text-sm text-gray-600">To be treated as Advance for</p>
                    <p className="text-sm font-medium text-gray-900">
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900">
                    {formatDate(feesStructure.startDate)} - {formatDate(feesStructure.endDate)}
                  </p>
                </div>
              </div>
              <div className="flex items-start space-x-3">
                <CalendarDays className="w-5 h-5 text-gray-400 mt-0.5" />
                <div>
                  <p className="text-sm text-gray-600">Online Collection Period</p>
                  <p className="text-sm font-medium text-gray-900">
              ...
              {feesStructure.numberOfInstalments && feesStructure.numberOfInstalments > 0 && (
                <div className="flex items-start space-x-3">
                  <CalendarDays className="w-5 h-5 text-gray-400 mt-0.5" />
                  <div>
            ...
            <div className="flex flex-wrap gap-2">
              {feesSlabMappings.map((slabMapping) => (
                <button
                  ...
                  onClick={() =>
                    setSelectedSlabId(
                      selectedSlabId === slabMapping.feesSlabId ? null : slabMapping.feesSlabId,
                    )
                  }
            ...
            <div className="flex-grow overflow-hidden border border-gray-300 rounded-lg flex flex-col">
              <table className="min-w-full h-[330px] flex flex-col">
                ...
                <tbody className="flex-grow overflow-y-auto bg-white">
                  ...
                  {Array.from({
                    length: Math.max(0, MIN_ROWS - feesStructure.components.length),
                  }).map((_, index) => (
                ...
                <tfoot className="flex-shrink-0">
                  <tr className="flex bg-gray-50">
                    <td className="flex-1 px-4 py-2 border-t border-r border-gray-300 text-right font-bold text-gray-900">
                      Total
                    </td>
