  ...
  const getModalContent = useMemo(() => {
    switch (type) {
      ...
      case "add":
        return (
          <div className="space-y-4">
            <div className="grid gap-4">{fieldConfig.map((field) => renderField(field))}</div>
          </div>
        );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">{getModalTitle}</DialogTitle>
