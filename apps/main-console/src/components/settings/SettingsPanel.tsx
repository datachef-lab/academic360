...
export default function SettingsPanel({
  ...
  return (
    <ul>
      <Accordion
        type="multiple"
        defaultValue={settingsCategories.map((_, index) => `panel-${index}`)}
      >
        {settingsCategories.map((panel, index) => (
          <AccordionItem
            key={`settings-panel-option-${index}`}
            value={`panel-${index}`}
            className="text-[14px]"
          >
            <AccordionTrigger>
            ...
            <AccordionContent
              className={`text-[14px] ${theme === "light" ? "text-slate-600" : "text-gray-300"} `}
            >
              {panel.tabs.length > 0 ? (
                <ul className="pl-4 ">
                  {panel.tabs.map((tab, tabIndex) => (
                    ...
                    >
                      <div className=" text-gray-600   drop-shadow-md">{tab.icon}</div>
