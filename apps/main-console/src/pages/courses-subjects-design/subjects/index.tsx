...
const SubjectsPage = () => {
  ...
  const handleDownloadTemplate = () => {
    ...
    XLSX.writeFile(wb, "subject-bulk-upload-template.xlsx");
  };
  ...
  const handleDownloadAll = async () => {
    try {
      ...
      toast.error("Failed to download subjects");
    }
  };
        ...
        <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center mb-3 justify-between gap-4 border rounded-md p-4 sticky top-0 z-30 bg-background">
          <div className="flex-1 min-w-0">
            <CardTitle className="flex items-center text-lg sm:text-xl">
              <BookOpen className="mr-2 h-6 w-6 sm:h-8 sm:w-8 border rounded-md p-1 border-slate-400 flex-shrink-0" />
              <span className="truncate">Subjects</span>
            </CardTitle>
            <div className="text-xs sm:text-sm text-muted-foreground mt-1">A list of all available subjects.</div>
          </div>
