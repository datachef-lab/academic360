...
export type SettingsRow = BloodGroup | Nationality | State | City | AnnualIncome | User | BoardUniversity | Institution | Category | Degree | LanguageMedium | Occupation | Qualification | Record<string, unknown>;
...
export async function fetchData({ activeResource, page = 1, pageSize = 10 }: { activeResource: { label: string; endpoint: string }; page: number; pageSize: number }, onEditRow?: (rowData: SettingsRow) => void) {
    ...
    switch (activeResource.label) {
        ...
        // case "Documents":
        // return { data: response.data as ApiResonse<PaginatedResponse<Document>>, columns: documentColumns(onEditRow as (rowData: Document) => void) };
        case "Blood Groups":
