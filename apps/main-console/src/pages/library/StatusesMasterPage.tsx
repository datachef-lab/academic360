...
const detailToForm = (d: LibraryStatusRow): FormState => ({
  name: d.name ?? "",
  isIssuable: Boolean(d.isIssuable),
  issuedTo: d.issuedTo ?? "",
});

const formToBody = (f: FormState): LibraryStatusUpsertBody => ({
  name: f.name.trim(),
  isIssuable: f.isIssuable,
  issuedTo: f.issuedTo.trim() ? f.issuedTo.trim() : null,
});
...
export default function StatusesMasterPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<LibraryStatusRow | null>(null);
  ...
  const fetchRows = useCallback(async () => {
    try {
      setLoading(true);
      const res = await getLibraryStatuses({
        page,
        limit,
  ...
  const openEdit = async (id: number) => {
    try {
      const res = await getLibraryStatusById(id);
      setEditingId(id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId == null) {
        await createLibraryStatus(body);
        toast.success("Status created");
      } else {
        await updateLibraryStatus(editingId, body);
  ...
  const openDeleteDialog = (row: LibraryStatusRow) => setDeleteTarget(row);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await deleteLibraryStatus(deleteTarget.id);
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[92vh] w-[min(96vw,720px)] overflow-y-auto sm:max-w-[720px]">
          <DialogHeader className="border-b px-6 py-4">
            <DialogTitle>{editingId == null ? "Add status" : "Edit status"}</DialogTitle>
          </DialogHeader>
          ...
          <div className="px-6 py-4">
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 rounded-md border border-slate-200 bg-muted/20 px-3 py-2">
                <Checkbox
                  checked={form.isIssuable}
                  onCheckedChange={(v) => setForm((f) => ({ ...f, isIssuable: v === true }))}
                />
                <Label className="text-sm font-medium">Issuable</Label>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="e.g. Student / Staff / Department"
                />
              </div>
            </div>
          </div>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <div className="space-y-2 text-left text-sm text-muted-foreground">
                ...
                <p>Deletion may fail if linked copy detail records exist.</p>
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
