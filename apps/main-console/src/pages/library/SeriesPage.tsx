...
import { Edit, Loader2, Search, Trash2, Workflow } from "lucide-react";
...
type LibrarySeriesSocketUpdate = {
  id: string;
...
export default function SeriesPage() {
  ...
  useEffect(() => {
    ...
    socket.emit("subscribe_library_series");

    const handleUpdate = (data: LibrarySeriesSocketUpdate) => {
      toast.info(data.message);
      void fetchRows();
    };

    socket.on("library_series_update", handleUpdate);

    return () => {
      socket.off("library_series_update", handleUpdate);
      socket.emit("unsubscribe_library_series");
    };
