...
type LibraryPeriodSocketUpdate = {
  id: string;
...
export default function PeriodsPage() {
  ...
  const [rows, setRows] = useState<LibraryPeriod[]>([]);
  ...
  const [selectedRow, setSelectedRow] = useState<LibraryPeriod | null>(null);
  const [formName, setFormName] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [deleteRow, setDeleteRow] = useState<LibraryPeriod | null>(null);
  ...
  const fetchRows = useCallback(async () => {
    try {
      setLoading(true);
      const response = await getLibraryPeriods({
        page,
        limit,
  ...
  useEffect(() => {
    ...
    socket.emit("subscribe_library_periods");

    const handleUpdate = (data: LibraryPeriodSocketUpdate) => {
      toast.info(data.message);
      void fetchRows();
    };

    socket.on("library_period_update", handleUpdate);

    return () => {
      socket.off("library_period_update", handleUpdate);
      socket.emit("unsubscribe_library_periods");
    };
  ...
  const openEditDialog = (row: LibraryPeriod) => {
    setSelectedRow(row);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = { name };
      if (selectedRow) {
        await updateLibraryPeriod(selectedRow.id, payload);
        toast.success("Period updated successfully");
      } else {
        await createLibraryPeriod(payload);
      ...
      console.error(error);
      toast.error(selectedRow ? "Failed to update period" : "Failed to create period");
    } finally {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setIsDeleting(true);
      await deleteLibraryPeriod(deleteRow.id);
    ...
    <div className="p-2 sm:p-4">
      ...
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{selectedRow ? "Edit Period" : "Add Period"}</DialogTitle>
          </DialogHeader>
