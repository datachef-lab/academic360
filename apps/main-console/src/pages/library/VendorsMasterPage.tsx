...
import {
  createLibraryVendor,
  deleteLibraryVendor,
  getLibraryVendorById,
  getLibraryVendors,
  updateLibraryVendor,
} from "@/services/library-vendors.service";
...
const detailToForm = (d: LibraryVendorRow): FormState => ({
  name: d.name ?? "",
  code: d.code ?? "",
  email: d.email ?? "",
  phone: d.phone ?? "",
  website: d.website ?? "",
  personOfContact: d.personOfContact ?? "",
  personOfContactEmail: d.personOfContactEmail ?? "",
  personOfContactPhone: d.personOfContactPhone ?? "",
  pan: d.pan ?? "",
});

const formToBody = (f: FormState): LibraryVendorUpsertBody => ({
  name: f.name.trim(),
  code: f.code.trim() || null,
  email: f.email.trim() || null,
  phone: f.phone.trim() || null,
  website: f.website.trim() || null,
  personOfContact: f.personOfContact.trim() || null,
  personOfContactEmail: f.personOfContactEmail.trim() || null,
  personOfContactPhone: f.personOfContactPhone.trim() || null,
  pan: f.pan.trim() || null,
});
...
export default function VendorsMasterPage() {
  ...
  const openEdit = async (id: number) => {
    try {
      const res = await getLibraryVendorById(id);
      setEditingId(id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId == null) {
        await createLibraryVendor(body);
        toast.success("Vendor created");
      } else {
        await updateLibraryVendor(editingId, body);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await deleteLibraryVendor(deleteTarget.id);
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-[11px] text-muted-foreground sm:text-sm">
                Suppliers from whom books and journals are purchased.
              </p>
            </div>
                      ...
                      <TableBody>
                        {rows.map((row, i) => (
                          <TableRow key={row.id}>
                            ...
                            <TableCell className="align-top text-sm">{row.email ?? "—"}</TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[92vh] w-[min(96vw,820px)] overflow-y-auto sm:max-w-[820px]">
          <DialogHeader className="border-b px-6 py-4">
            <DialogTitle>{editingId == null ? "Add vendor" : "Edit vendor"}</DialogTitle>
          </DialogHeader>
          ...
          <div className="grid grid-cols-1 gap-4 px-6 py-4 sm:grid-cols-2">
            ...
            <div className="space-y-1.5 sm:col-span-2">
              <div className="border-t pt-4 text-sm font-semibold text-slate-700">
                Person of contact
              </div>
            </div>
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <Input
                ...
                onChange={(e) => setForm((f) => ({ ...f, personOfContact: e.target.value }))}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Email</Label>
              <Input
                type="email"
                value={form.personOfContactEmail}
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    personOfContactEmail: e.target.value,
                  }))
                }
              />
            </div>
            <div className="space-y-1.5">
              <Label>Phone</Label>
              <Input
                value={form.personOfContactPhone}
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    personOfContactPhone: e.target.value,
                  }))
                }
              />
            </div>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <div className="space-y-2 text-left text-sm text-muted-foreground">
                ...
                <p>Deletion may fail if this vendor is linked to any copy details.</p>
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
