...
import { BookText, Edit, Loader2, Plus, Search, Trash2 } from "lucide-react";
...
const detailToForm = (d: JournalTypeRow): FormState => ({
  name: d.name ?? "",
});

const formToBody = (f: FormState): JournalTypeUpsertBody => ({
  name: f.name.trim(),
});
...
export default function JournalTypesMasterPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<JournalTypeRow | null>(null);
  ...
  const fetchRows = useCallback(async () => {
    try {
      setLoading(true);
      const res = await getJournalTypes({
        page,
        limit,
  ...
  const openEdit = async (id: number) => {
    try {
      const res = await getJournalTypeById(id);
      setEditingId(id);
      setForm(detailToForm(res.payload));
      setDialogOpen(true);
    } catch (e) {
      console.error(e);
      toast.error("Failed to load journal type");
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId == null) {
        await createJournalType(body);
        toast.success("Journal type created");
      } else {
        await updateJournalType(editingId, body);
      ...
      console.error(e);
      toast.error("Could not save journal type");
    } finally {
      setSaving(false);
    }
  };

  const openDeleteDialog = (row: JournalTypeRow) => setDeleteTarget(row);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await deleteJournalType(deleteTarget.id);
    ...
    <div className="min-w-0 p-2 sm:p-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <CardTitle className="flex items-center text-lg sm:text-xl">
                <BookText className="mr-2 h-8 w-8 rounded-md border  p-1" />
                Journal Types
              </CardTitle>
              <p className="mt-1 text-[11px] text-muted-foreground sm:text-sm">
                Maintain journal type master data.
              </p>
            </div>
              ...
              <>
                ...
                <div className="hidden min-w-0 pb-2 lg:block">
                  <div className="rounded-md border bg-background">
                    <Table containerClassName="min-w-[780px]">
                      <TableHeader>
                        <TableRow>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[92vh] w-[min(96vw,720px)] overflow-y-auto sm:max-w-[720px]">
          <DialogHeader className="border-b px-6 py-4">
            <DialogTitle>
              {editingId == null ? "Add journal type" : "Edit journal type"}
            </DialogTitle>
          </DialogHeader>
      ...
      >
        <AlertDialogContent className="max-w-lg">
          <AlertDialogHeader>
            <AlertDialogTitle>Delete journal type?</AlertDialogTitle>
            <AlertDialogDescription>
              <div className="space-y-2 text-left text-sm text-muted-foreground">
                ...
                <p>Deletion may fail if journals are linked to this type.</p>
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
