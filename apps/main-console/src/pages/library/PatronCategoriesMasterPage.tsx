...
} from "@/services/library-patron-categories.service";
import {
  createLibraryPatronCategory,
  deleteLibraryPatronCategory,
  getLibraryPatronCategoryById,
  getLibraryPatronCategories,
  updateLibraryPatronCategory,
} from "@/services/library-patron-categories.service";
import {
  STICKY_THEAD_CLASS,
  STICKY_TH_BASE,
...
const detailToForm = (d: LibraryPatronCategoryRow): FormState => ({
  name: d.name ?? "",
  code: d.code ?? "",
...
const formToBody = (f: FormState): LibraryPatronCategoryUpsertBody => ({
  name: f.name.trim(),
...
export default function PatronCategoriesMasterPage() {
  const [rows, setRows] = useState<LibraryPatronCategoryRow[]>([]);
  ...
  const [deleteTarget, setDeleteTarget] = useState<LibraryPatronCategoryRow | null>(null);
  ...
  const fetchRows = useCallback(async () => {
    try {
      setLoading(true);
      const res = await getLibraryPatronCategories({
        page,
        limit,
      ...
      console.error(e);
      toast.error("Failed to load patron categories");
    } finally {
      setLoading(false);
    }
  ...
  const openEdit = async (id: number) => {
    try {
      const res = await getLibraryPatronCategoryById(id);
      setEditingId(id);
      setForm(detailToForm(res.payload));
      setDialogOpen(true);
    } catch (e) {
      console.error(e);
      toast.error("Failed to load patron category");
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId == null) {
        await createLibraryPatronCategory(body);
        toast.success("Patron category created");
      } else {
        await updateLibraryPatronCategory(editingId, body);
        toast.success("Patron category updated");
      }
      setDialogOpen(false);
      void fetchRows();
    } catch (e) {
      console.error(e);
      toast.error("Could not save patron category");
    } finally {
      setSaving(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await deleteLibraryPatronCategory(deleteTarget.id);
      toast.success("Patron category deleted");
      closeDeleteDialog();
      void fetchRows();
    } catch (e) {
      console.error(e);
    ...
    <div className="min-w-0 p-2 sm:p-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <CardTitle className="flex items-center text-lg sm:text-xl">
                <IdCard className="mr-2 h-8 w-8 rounded-md border p-1" />
                Patron Categories
              </CardTitle>
              <p className="mt-1 text-[11px] text-muted-foreground sm:text-sm">
                Borrower categories (Student, Faculty, Staff…) used by the circulation policy grid.
              </p>
            </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[92vh] w-[min(96vw,640px)] overflow-y-auto sm:max-w-[640px]">
          <DialogHeader className="border-b px-6 py-4">
            <DialogTitle>
              {editingId == null ? "Add patron category" : "Edit patron category"}
            </DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-1 gap-4 px-6 py-4 sm:grid-cols-2">
            ...
            <div className="flex items-center gap-2 sm:col-span-2">
              ...
              />
              <Label htmlFor="patron-active" className="text-sm font-normal">
                Active
              </Label>
            </div>
          </div>
      ...
      >
        <AlertDialogContent className="max-w-lg">
          <AlertDialogHeader>
            <AlertDialogTitle>Delete patron category?</AlertDialogTitle>
            <AlertDialogDescription>
