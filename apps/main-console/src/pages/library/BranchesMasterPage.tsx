...
import { Building, Loader2, Edit, Plus, Search, Trash2 } from "lucide-react";
import type {
  LibraryBranchRow,
  LibraryBranchUpsertBody,
...
const detailToForm = (d: LibraryBranchRow): FormState => ({
  name: d.name ?? "",
  code: d.code ?? "",
...
const formToBody = (f: FormState): LibraryBranchUpsertBody => ({
  name: f.name.trim(),
...
export default function BranchesMasterPage() {
  const [rows, setRows] = useState<LibraryBranchRow[]>([]);
  ...
  const [deleteTarget, setDeleteTarget] = useState<LibraryBranchRow | null>(null);
  ...
  const fetchRows = useCallback(async () => {
    try {
      setLoading(true);
      const res = await getLibraryBranches({
        page,
        limit,
  ...
  const openEdit = async (id: number) => {
    try {
      const res = await getLibraryBranchById(id);
      setEditingId(id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId == null) {
        await createLibraryBranch(body);
        toast.success("Branch created");
      } else {
        await updateLibraryBranch(editingId, body);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await deleteLibraryBranch(deleteTarget.id);
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <CardTitle className="flex items-center text-lg sm:text-xl">
                <Building className="mr-2 h-8 w-8 rounded-md border p-1" />
                Branches
              </CardTitle>
              <p className="mt-1 text-[11px] text-muted-foreground sm:text-sm">
                Library locations across campuses.
              </p>
            </div>
              ...
              <>
                <div className="min-w-0 pb-2">
                  <div className="max-h-[70vh] overflow-auto [&>div]:!overflow-visible rounded-md border bg-background">
                    <Table containerClassName="min-w-[630px]">
                        ...
                        {rows.map((row, i) => (
                          ...
                          >
                            ...
                            <TableCell className="align-top text-sm">
                              {row.openingDate ? parseDate(row.openingDate) : "—"}
                            </TableCell>
                            <TableCell className="align-top text-sm">
                              <span
                                className={
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[92vh] w-[min(96vw,820px)] overflow-y-auto sm:max-w-[820px]">
          <DialogHeader className="border-b px-6 py-4">
            <DialogTitle>{editingId == null ? "Add branch" : "Edit branch"}</DialogTitle>
          </DialogHeader>
          ...
          <div className="grid grid-cols-1 gap-4 px-6 py-4 sm:grid-cols-2">
            ...
            <div className="rounded-md border border-dashed bg-muted/30 p-3 text-xs text-muted-foreground sm:col-span-2">
              Address, phone, and email are stored on the shared address record (linked via{" "}
              <code>address.library_branch_id_fk</code>). Manage those from the address editor for
              this branch.
            </div>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <div className="space-y-2 text-left text-sm text-muted-foreground">
                ...
                <p>
                  Deletion will fail if any book, copy, circulation, or entry log is linked to this
                  branch.
                </p>
              </div>
            </AlertDialogDescription>
