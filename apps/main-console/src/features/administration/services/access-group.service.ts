...
export async function getAllAccessGroups() {
  const { data } =
    await axiosInstance.get<ApiResponse<AccessGroupDto[]>>(BASE_URL);
  return data;
}

export async function getAccessGroupById(id: number) {
  const { data } = await axiosInstance.get<ApiResponse<AccessGroupDto>>(
    `${BASE_URL}/${id}`,
  );
  return data;
}
...
export async function createAccessGroup(payload: AccessGroupCreateInput) {
  const { data } = await axiosInstance.post<ApiResponse<AccessGroupDto>>(
    BASE_URL,
    payload,
  );
  return data;
...
export async function updateAccessGroup(
  id: number,
  payload: AccessGroupUpdateInput,
) {
