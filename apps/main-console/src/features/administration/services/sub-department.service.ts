import type { SubDepartment } from "@repo/db";
...
export async function getAllSubDepartments() {
  const { data } = await axiosInstance.get<ApiResponse<SubDepartment[]>>(BASE_URL);
  return data;
}

export async function createSubDepartment(payload: SubDepartmentPayload) {
  const { data } = await axiosInstance.post<ApiResponse<SubDepartment>>(BASE_URL, payload);
  return data;
}

export async function updateSubDepartment(id: number, payload: SubDepartmentPayload) {
  const { data } = await axiosInstance.put<ApiResponse<SubDepartment>>(`${BASE_URL}/${id}`, payload);
  return data;
}
