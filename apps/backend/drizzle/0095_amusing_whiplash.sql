CREATE TABLE "temp_admit_card_distributions" (
	...
	"exam_candidate_id_fk" integer NOT NULL,
	"distributed_by_user_id_fk" integer NOT NULL,
	"created_at" timestamp with time zone DEFAULT now(),
	"updated_at" timestamp with time zone DEFAULT now()
);
--> statement-breakpoint
ALTER TABLE "temp_admit_card_distributions" ADD CONSTRAINT "temp_admit_card_distributions_exam_candidate_id_fk_exam_candidates_id_fk" FOREIGN KEY ("exam_candidate_id_fk") REFERENCES "public"."exam_candidates"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
