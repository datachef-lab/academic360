...
} from "@/features/resources/models/boardResultStatus.model.js";
...
export async function findAllBoardResultStatuses(): Promise<
  BoardResultStatus[]
> {
  return await db
    .select()
    .from(boardResultStatusModel)
    .orderBy(boardResultStatusModel.sequence);
}

export async function createBoardResultStatus(
  data: Omit<BoardResultStatus, "id" | "createdAt" | "updatedAt">,
): Promise<BoardResultStatus> {
...
export async function updateBoardResultStatus(
  id: number,
  data: Partial<Omit<BoardResultStatus, "id" | "createdAt" | "updatedAt">>,
): Promise<BoardResultStatus | null> {
...
export async function deleteBoardResultStatus(
  id: number,
): Promise<BoardResultStatus | null> {
...
export async function findBoardResultStatusByName(
  name: string,
): Promise<BoardResultStatus | null> {
  const [foundBoardResultStatus] = await db
    .select()
