...
import { Transport, transportModel } from "@academic/db/schemas";
...
export async function createTransport(
  data: Omit<Transport, "id" | "createdAt" | "updatedAt">,
): Promise<Transport> {
...
export async function updateTransport(
  id: number,
  data: Partial<Omit<Transport, "id" | "createdAt" | "updatedAt">>,
...
export async function findTransportsByMode(
  mode:
    | "BUS"
    | "TRAIN"
    | "METRO"
  ...
  return await db
    .select()
    .from(transportModel)
    .where(eq(transportModel.mode, mode))
    .orderBy(transportModel.id);
}

export async function findTransportByVehicleNumber(
  vehicleNumber: string,
): Promise<Transport | null> {
  const [foundTransport] = await db
    .select()
    .from(transportModel)
    .where(eq(transportModel.vehicleNumber, vehicleNumber.trim()));
  return foundTransport;
}
