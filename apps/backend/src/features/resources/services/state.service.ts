...
import { State, stateModel } from "@academic/db/schemas";
...
export async function updateState(
  id: number,
  data: Partial<Omit<State, "id" | "createdAt" | "updatedAt">>,
): Promise<State | null> {
...
export async function findStateByName(name: string): Promise<State | null> {
  const [foundState] = await db
    .select()
    .from(stateModel)
    .where(eq(stateModel.name, name.toUpperCase().trim()));
  return foundState;
}

export async function findStatesByCountryId(
  countryId: number,
): Promise<State[]> {
  return await db
    .select()
    .from(stateModel)
    .where(eq(stateModel.countryId, countryId))
    .orderBy(stateModel.sequence);
}
