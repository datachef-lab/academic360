...
export const createNewLanguageMedium = async (
  ...
  try {
    ...
    if (existingName) {
      res
        .status(409)
        .json(new ApiError(409, "Language medium name already exists"));
      return;
    }
    ...
    const newLanguageMedium =
      await createLanguageMediumService(languageMediumData);
...
export const updateLanguageMedium = async (
  ...
  try {
    ...
    const updateData: Partial<
      Omit<LanguageMedium, "id" | "createdAt" | "updatedAt">
    > = {};
    ...
    const updatedLanguageMedium = await updateLanguageMediumService(
      Number(id),
      updateData,
    );
