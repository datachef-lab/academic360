...
export const createBoardResultStatus = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    ...
    if (!spclType || typeof spclType !== "string") {
      res
        .status(400)
        .json(
          new ApiError(400, "Special type is required and must be a string"),
        );
      return;
    }
    ...
    if (result && !["FAIL", "PASS"].includes(result)) {
      res
        .status(400)
        .json(new ApiError(400, "Result must be either 'FAIL' or 'PASS'"));
      return;
    }
    ...
    if (existingName) {
      res
        .status(409)
        .json(new ApiError(409, "Board result status name already exists"));
      return;
    }
    ...
    const newBoardResultStatus = await createBoardResultStatusService(
      boardResultStatusData,
    );
...
export const getAllBoardResultStatuses = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
...
export const getBoardResultStatusById = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
...
export const updateBoardResultStatus = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    ...
    const existingBoardResultStatus = await findBoardResultStatusById(
      Number(id),
    );
    ...
    const updateData: Partial<
      Omit<BoardResultStatus, "id" | "createdAt" | "updatedAt">
    > = {};
    ...
    const updatedBoardResultStatus = await updateBoardResultStatusService(
      Number(id),
      updateData,
    );
...
export const deleteBoardResultStatus = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    ...
    const deletedBoardResultStatus = await deleteBoardResultStatusService(
      Number(id),
    );
