...
export const createAnnualIncomeController = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    ...
    if (!range || typeof range !== "string") {
      res
        .status(400)
        .json(new ApiError(400, "Range is required and must be a string"));
      return;
    }
    ...
    if (existingRange) {
      res
        .status(409)
        .json(new ApiError(409, "Annual income range already exists"));
      return;
    }
...
export const getAllAnnualIncomes = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
...
export const updateAnnualIncomeController = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    ...
    const updateData: Partial<
      Omit<AnnualIncome, "id" | "createdAt" | "updatedAt">
    > = {};
    ...
    const updatedAnnualIncome = await updateAnnualIncome(
      Number(id),
      updateData,
    );
...
export const deleteAnnualIncomeController = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
