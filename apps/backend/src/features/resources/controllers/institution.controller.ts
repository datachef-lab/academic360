...
export const createNewInstitution = async (
  ...
  try {
    ...
    if (existingName) {
      res
        .status(409)
        .json(new ApiError(409, "Institution name already exists"));
      return;
    }
...
export const updateInstitution = async (
  ...
  try {
    ...
    if (addressId !== undefined)
      updateData.addressId = addressId ? Number(addressId) : null;
    if (sequence !== undefined) updateData.sequence = sequence;
    if (disabled !== undefined) updateData.disabled = disabled;

    const updatedInstitution = await updateInstitutionService(
      Number(id),
      updateData,
    );
