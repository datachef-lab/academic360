...
export const createBoardUniversity = async (
  ...
  try {
    ...
    if (name.length > 700) {
      res
        .status(400)
        .json(new ApiError(400, "Name must be less than 700 characters"));
      return;
    }
    ...
    if (existingName) {
      res
        .status(409)
        .json(new ApiError(409, "Board university name already exists"));
      return;
    }
    ...
    if (code) {
      ...
      if (existingCode) {
        res
          .status(409)
          .json(new ApiError(409, "Board university code already exists"));
        return;
      }
    }
    ...
    if (
      ...
      (isNaN(passingMarks) || passingMarks < 0 || passingMarks > 100)
    ) {
      res
        .status(400)
        .json(
          new ApiError(400, "Passing marks must be a number between 0 and 100"),
        );
      return;
    }
    ...
    const newBoardUniversity =
      await createBoardUniversityService(boardUniversityData);
...
export const updateBoardUniversity = async (
  ...
  try {
    ...
    const updateData: Partial<
      Omit<BoardUniversity, "id" | "createdAt" | "updatedAt">
    > = {};
    ...
    const updatedBoardUniversity = await updateBoardUniversityService(
      Number(id),
      updateData,
    );
...
export const deleteBoardUniversity = async (
  ...
  try {
    ...
    const deletedBoardUniversity = await deleteBoardUniversityService(
      Number(id),
    );
