...
export const createClassController = async (
  ...
  try {
    ...
    res
      .status(201)
      .json(
        new ApiResponse(201, "SUCCESS", newClass, "Class created successfully"),
      );
  } catch (error) {
...
export const getClassByIdController = async (
  ...
  try {
    ...
    if (!foundClass) {
      res
        .status(404)
        .json(new ApiResponse(404, "FAIL", null, "Class not found"));
      return;
    }
    res
      .status(200)
...
export const deleteClassController = async (
  ...
  try {
    ...
    res
      .status(200)
      .json(
        new ApiResponse(200, "SUCCESS", null, "Class deleted successfully"),
      );
  } catch (error) {
    handleError(error, res, next);
  }
