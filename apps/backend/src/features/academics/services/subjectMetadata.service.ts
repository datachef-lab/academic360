...
export async function uploadSubjects(fileName: string): Promise<boolean> {
    ...
    for (let i = 0; i < subjectArr.length; i++) {
        ...
        const irpType = subjectArr[i]["Subject Type as per IRP"]?.toUpperCase().trim();
        const marksheetType = subjectArr[i]["Subject Type as per Marksheet"]?.toUpperCase().trim();
        ...
        const [foundSubjectType] = await db
            ...
            .where(whereClauses.length === 2 ? and(...whereClauses) : whereClauses[0]);
...
export async function refactorSubejectsWithSubjectMetadata(fileName: string): Promise<boolean> {
    // Read the file from the `/public/temp/` directory
    const filePath = path.resolve(directoryName, "../../../..", "public", "temp", fileName);
...
export async function refactorSubjectIrpCode() {
    ...
    console.log(`Found ${subjectMetadatas.length} subject metadatas to refactor irpCode.`);

    for (const subjectMetadata of subjectMetadatas) {
        const dashIndex = subjectMetadata.marksheetCode.indexOf("-");
        console.log("dashIndex:", dashIndex, subjectMetadata.marksheetCode, subjectMetadata.irpCode);
        // if (subjectMetadata.irpCode == null && dashIndex != -1) {
        const irpCode = subjectMetadata.marksheetCode.substring(0, dashIndex);
        console.log(`Updating subject metadata with id ${subjectMetadata.id} with irpCode: ${irpCode}`);

        const [updatedSbjMetadata] = await db.update(subjectMetadataModel)
            .set({ irpCode })
            .where(eq(subjectMetadataModel.id, subjectMetadata.id!)).returning();
