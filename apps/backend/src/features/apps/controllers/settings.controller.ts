...
export const getSettingsByIdHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (req.params.id) {
...
export const updateSettingByIdHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
...
export const downloadSettingFileHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { idOrName } = req.params;

        if (!idOrName) {
            return res.status(400).json(new ApiResponse(400, "BAD_REQUEST", "Setting ID or name required"));
        }

        const setting = await findByIdOrName(idOrName);

        if (!setting) {
            return res.status(404).json(new ApiResponse(404, "NOT_FOUND", "Setting not found"));
        }

        if (setting.type !== "FILE" || !setting.value) {
            return res.status(400).json(new ApiResponse(400, "BAD_REQUEST", "Setting is not a file type or value is empty"));
        }

        const filePath = path.join(SETTINGS_PATH, setting.value);

        if (!fs.existsSync(filePath)) {
            return res.status(404).json(new ApiResponse(404, "NOT_FOUND", "File not found on server"));
        }
