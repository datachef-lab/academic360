...
export async function findAdmCourseDetailsByStudentId(studentId: number): Promise<AdmissionCourseDetailsDto | null> {
...
async function modelToDto(model: AdmissionCourseDetails): Promise<AdmissionCourseDetailsDto> {
    ...
    const formattedAdmCourseDetails: AdmissionCourseDetailsDto = {
        ...
        studentCategory: studentCategoryId ? await findStudentCategoryById(studentCategoryId) : null,
    };
...
async function findProgramCourseById(id: number): Promise<ProgramCourseDto | null> {
    if (!id) return null;
    const [programCourse] = await db.select().from(programCourseModel).where(eq(programCourseModel.id, id));
    return programCourse ? await programCourseResponseFormat(programCourse) : null;
}

async function programCourseResponseFormat(programCourse: ProgramCourse): Promise<ProgramCourseDto> {
    const { courseId, courseTypeId, courseLevelId, affiliationId, regulationTypeId, streamId, ...rest } = programCourse;
    return {
        ...rest,
        stream: streamId ? await findStreamById(streamId) : null,
        course: courseId ? await findCourseById(courseId) : null,
        courseType: courseTypeId ? await findCourseTypeById(courseTypeId) : null,
        courseLevel: courseLevelId ? await findCourseLevelById(courseLevelId) : null,
        affiliation: affiliationId ? await findAffiliationById(affiliationId) : null,
        regulationType: regulationTypeId ? await findRegulationTypeById(regulationTypeId) : null,
    };
}

async function findClassById(id: number): Promise<Class | null> {
    if (!id) return null;
    const [classData] = await db.select().from(classModel).where(eq(classModel.id, id));
    return classData || null;
}

async function findShiftById(id: number): Promise<Shift | null> {
    if (!id) return null;
    const [shift] = await db.select().from(shiftModel).where(eq(shiftModel.id, id));
    return shift || null;
}

async function findEligibilityCriteriaById(id: number): Promise<EligibilityCriteria | null> {
    if (!id) return null;
    const [eligibilityCriteria] = await db.select().from(eligibilityCriteriaModel).where(eq(eligibilityCriteriaModel.id, id));
    return eligibilityCriteria || null;
}

async function findStudentCategoryById(id: number): Promise<StudentCategory | null> {
    if (!id) return null;
    const [studentCategory] = await db.select().from(studentCategoryModel).where(eq(studentCategoryModel.id, id));
...
async function findCourseById(id: number): Promise<Course | null> {
    if (!id) return null;
    const [course] = await db.select().from(courseModel).where(eq(courseModel.id, id));
    return course || null;
}
...
async function findRegulationTypeById(id: number): Promise<RegulationType | null> {
    if (!id) return null;
