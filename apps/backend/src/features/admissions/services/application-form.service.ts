...
} from "./admission-general-info.service.js";
import {
  deleteAcademicInfo,
  findAcademicInfoByApplicationFormId,
} from "./admission-academic-info.service.js";
import {
...
} from "./admission-course-application.service.js";
...
export async function createApplicationForm(
  ...
  if (existingEntry) {
    return {
      ...
      message:
        "Application form already exists for this student. Try login to continue.",
    };
  }
...
export async function findApplicationFormsByAdmissionId(admissionId: number) {
  ...
  const dtos = (
    await Promise.all(forms.map(async (form) => await formatAppform(form)))
  ).filter((form): form is ApplicationFormDto => form !== null);
...
export async function updateApplicationForm(
  id: number,
  givenForm: Partial<ApplicationForm>,
) {
...
export async function deleteApplicationForm(id: number) {
  ...
  const generalInfoDeleted = await deleteGeneralInfo(
    foundForm.generalInfo!.id!,
  );
  ...
  const academicInfoDeleted = await deleteAcademicInfo(
    foundForm.academicInfo!.id!,
  );
  ...
  for (const course of foundForm.courseApplication!) {
    const courseApplicationInfoDeleted = await deleteAdmissionCourse(
      course.id!,
    );
    if (
      courseApplicationInfoDeleted !== null &&
      !courseApplicationInfoDeleted
    ) {
  ...
  if (foundForm.additionalInfo) {
    const additionalInfoDeleted = await deleteAdmissionAdditionalInfo(
      foundForm.additionalInfo!.id!,
    );
...
// FORMAT DTO
export async function formatAppform(
  form: ApplicationForm,
