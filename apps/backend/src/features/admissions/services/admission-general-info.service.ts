...
} from "../models/admission-general-info.model.js";
...
export async function createGeneralInfo(
  ...
  const admission = await findAdmissionById(
    Number(applicationForm?.admissionId!),
  );
  if (!admission) {
  ...
  if (existingEntry) {
    return {
      generalInfo: existingEntry,
      message: "General info already exists for this student.",
    };
  }
  ...
  return {
    ...
    message: "New General Info Created!",
  };
}

// LOGIN
export async function findByLoginIdAndPassword(
  mobileNumber: string,
  password: string,
) {
...
export async function findGeneralInfoByApplicationFormId(
...
export async function updateGeneralInfo(
  generalInfo: Omit<AdmissionGeneralInfo, "password">,
) {
...
export async function checkExistingEntry(
  admissionId: number,
  generalInfo: Partial<AdmissionGeneralInfo>,
) {
  const [existingEntry] = await db
    ...
    .where(
      and(
        ilike(
          admissionGeneralInfoModel.mobileNumber,
          String(generalInfo.mobileNumber || "").trim(),
        ),
