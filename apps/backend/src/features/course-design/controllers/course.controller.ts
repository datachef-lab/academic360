...
import {
  ...
  updateCourse,
  bulkUploadCourses,
} from "@/features/course-design/services/course.service.js";
import { ApiResponse } from "@/utils/ApiResonse.js";
import { socketService } from "@/services/socketService.js";

export async function bulkUploadCoursesHandler(
  req: Request,
  res: Response,
): Promise<void> {
  try {
...
export async function getAllCoursesHandler(
  req: Request,
  res: Response,
): Promise<void> {
  try {
    ...
    res
      .status(500)
      .json(new ApiResponse(500, "ERROR", null, "Failed to fetch courses"));
    return;
  }
}
...
export async function getCourseByIdHandler(
  ...
  try {
    ...
    if (isNaN(id)) {
      res
        .status(400)
        .json(new ApiResponse(400, "BAD_REQUEST", null, "Invalid course ID"));
      return;
    }
    ...
    if (!course) {
      res
        .status(404)
        .json(new ApiResponse(404, "NOT_FOUND", null, "Course not found"));
      return;
    }
    ...
    res
      .status(200)
      .json(
        new ApiResponse(200, "SUCCESS", course, "Course fetched successfully"),
      );
    return;
  } catch (error) {
    ...
    res
      .status(500)
      .json(new ApiResponse(500, "ERROR", null, "Failed to fetch course"));
    return;
  }
}
...
export async function createCourseHandler(
  ...
  try {
    ...
    res
      .status(500)
      .json(new ApiResponse(500, "ERROR", null, "Failed to create course"));
    return;
  }
}
...
export async function updateCourseHandler(
  ...
  try {
    ...
    res
      .status(500)
      .json(new ApiResponse(500, "ERROR", null, "Failed to update course"));
    return;
  }
}
...
export async function deleteCourseHandler(
  ...
  try {
    ...
    res
      .status(500)
      .json(new ApiResponse(500, "ERROR", null, "Failed to delete course"));
    return;
  }
}
  ...
  try {
    ...
    if (!query || query.trim() === "") {
      return res
        .status(400)
        .json(
          new ApiResponse(400, "BAD_REQUEST", null, "Search query is required"),
        );
    }
    ...
    return res
      .status(500)
      .json(new ApiResponse(500, "ERROR", null, "Failed to search courses"));
  }
}
