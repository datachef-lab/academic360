...
import {
  ...
  deleteStreamSafe,
  bulkUploadStreams,
} from "@/features/course-design/services/stream.service.js";
  ...
  try {
    ...
    if (!created) {
      res
        .status(400)
        .json(
          new ApiResponse(400, "BAD_REQUEST", null, "Stream already exists"),
        );
      return;
    }
    res
...
export const bulkUploadStreamsHandler = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const getStreamByIdHandler = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    res
      .status(200)
      .json(
        new ApiResponse(200, "SUCCESS", stream, "Stream fetched successfully"),
      );
  } catch (error) {
    handleError(error, res, next);
  }
...
export const getAllStreamsHandler = async (
  _req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, "SUCCESS", streams, "All streams fetched"));
  } catch (error) {
    handleError(error, res, next);
  }
};
...
export const updateStreamHandler = async (
  ...
  try {
    ...
    if (!updated) {
      res
        .status(404)
        .json(new ApiResponse(404, "NOT_FOUND", null, "Stream not found"));
      return;
    }
    res
      .status(200)
      .json(
        new ApiResponse(200, "UPDATED", updated, "Stream updated successfully"),
      );
  } catch (error) {
    handleError(error, res, next);
  }
...
export const deleteStreamHandler = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
