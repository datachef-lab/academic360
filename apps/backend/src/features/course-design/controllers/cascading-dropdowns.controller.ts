...
export const getAvailableAffiliationsHandler = async (
  req: Request,
  res: Response,
) => {
  try {
...
export const getAcademicYearsByAffiliationHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    const academicYears = await getAcademicYearsByAffiliation(
...
export const getRegulationTypesByAffiliationAndAcademicYearHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    const regulationTypes =
      await getRegulationTypesByAffiliationAndAcademicYear(
        Number(affiliationId),
    ...
    console.error(
      "Error in getRegulationTypesByAffiliationAndAcademicYearHandler:",
      error,
    );
    res.status(500).json({
      success: false,
...
export const getSubjectsByAffiliationAcademicYearAndRegulationHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (!affiliationId || !academicYearId || !regulationTypeId) {
      res.status(400).json({
        success: false,
        message:
          "Affiliation ID, Academic Year ID, and Regulation Type ID are required",
      });
      return;
    }
    ...
    console.error(
      "Error in getSubjectsByAffiliationAcademicYearAndRegulationHandler:",
      error,
    );
    res.status(500).json({
      success: false,
