...
export const createPaperComponent = async (
  paperComponentData: PaperComponentDto,
) => {
  const { id, createdAt, updatedAt, examComponent, ...props } =
    paperComponentData;
  ...
  if (!existingPaperComponent) {
    const [newPaperComponent] = await db
      .insert(paperComponentModel)
      .values({
        ...props,
...
export const updatePaperComponent = async (
  id: string,
  paperComponentData: PaperComponentDto,
) => {
  const {
  ...
  const paperComponent =
    updatedPaperComponent.length > 0 ? updatedPaperComponent[0] : null;
...
export const deletePaperComponentSafe = async (id: string) => {
  ...
  const [{ mksCompCount }] = await db
    .select({
      mksCompCount: countDistinct(marksheetPaperComponentMappingModel.id),
    })
  ...
  if (mksCompCount > 0) {
    return {
      success: false,
      message:
        "Cannot delete paper-component. It is associated with other records.",
  ...
  if (deletedPaperComponent.length > 0) {
    return {
      success: true,
      message: "Paper-component deleted successfully.",
      records: [],
    };
  }
  return {
    success: false,
    message: "Failed to delete paper-component.",
    records: [],
  };
};
...
): Promise<PaperComponentDto | null> {
  const { examComponentId, ...props } = paperComponent;

  const examComponent = (await findExamComponentById(
    examComponentId.toString(),
  ))!;
