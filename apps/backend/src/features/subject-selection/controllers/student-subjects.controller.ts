...
export async function getStudentMandatoryPapers(req: Request, res: Response) {
  try {
    const studentId = Number(req.params.studentId);
    if (!studentId || Number.isNaN(studentId)) {
      return res
    ...
    const mandatoryPapers = await findMandatoryPapers(studentId);
    return res
      .status(200)
      .json(
        new ApiResponse(200, "OK", mandatoryPapers, "Fetched mandatory papers"),
      );
  } catch (error: any) {
    return res
      .status(500)
