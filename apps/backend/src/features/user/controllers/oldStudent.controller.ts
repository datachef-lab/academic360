...
import { OldCourseDetails, OldStudentCategory } from "@repo/db/legacy-system-types/admissions";
...
import { OldCvSubjectSelection, OldMeritList } from "@repo/db/legacy-system-types/admissions";
...
import { addCity, addCountry, addDistrict, addState, loadData } from "../services/old-student.service";
...
export async function addBloodGroup(type: string, db: DbType, legacyBloodGroupId: number) {
    const [existingBloodGroup] = await db.select().from(bloodGroupModel).where(ilike(bloodGroupModel.type, type.trim().toUpperCase()));
...
export async function addNationality(name: string, code: number | undefined | null, db: DbType, legacyNationalityId?: number) {
    const [existingNationality] = await db.select().from(nationalityModel).where(ilike(nationalityModel.name, name.trim()));
...
export async function addReligion(name: string, db: DbType, legacyReligionId: number) {
    const [existingReligion] = await db.select().from(religionModel).where(ilike(religionModel.name, name.trim().toUpperCase()));
    if (existingReligion) {
...
export async function addUserForStudent(oldStudent: OldStudent, db: DbType, type: "STUDENT") {
...
export async function addAccommodation(oldStudent: OldStudent, student: Student, accommodationId?: number) {
    ...
    if (accommodationId) {
        ...
        const [existingAccommodation] = await db.select().from(accommodationModel).where(eq(accommodationModel.id, accommodationId));
        ...
        const [updatedAccommodation] = await db.update(accommodationModel)
            .set({
                placeOfStay,
            })
...
export async function addFamily(oldStudent: OldStudent, student: Student, familyId?: number) {
    // If familyId is provided, perform update and return
    if (familyId) {
        const [existingFamily] = await db.select().from(familyModel).where(eq(familyModel.id, familyId));
        if (!existingFamily) {
            return null;
        }
        ...
        if (existingFamily.fatherDetailsId) {
            ...
            await db.update(personModel)
                .set({
                    name: oldStudent.fatherName?.toUpperCase()?.trim(),
                    email: oldStudent.fatherEmail?.trim().toLowerCase(),
                    aadhaarCardNumber: formatAadhaarCardNumber(oldStudent.fatheraadharno || undefined),
        ...
        if (existingFamily.motherDetailsId) {
            ...
            await db.update(personModel)
                .set({
                    ...
                    aadhaarCardNumber: formatAadhaarCardNumber(oldStudent.motheraadharno || undefined),
        ...
        if (existingFamily.guardianDetailsId) {
            ...
            await db.update(personModel)
                .set({
                    ...
                    aadhaarCardNumber: formatAadhaarCardNumber(oldStudent.gurdianaadharno || undefined),
        ...
        const [updatedFamily] = await db.update(familyModel)
            .set({
...
export async function addHealth(oldStudent: OldStudent, student: Student, healthId?: number) {
    // If healthId is provided, perform update and return
    if (healthId) {
        const [existingHealth] = await db.select().from(healthModel).where(eq(healthModel.id, healthId));
        if (!existingHealth) {
            return null;
        }
        ...
        const [updatedHealth] = await db.update(healthModel)
            .set({
...
export async function addEmergencyContact(oldStudent: OldStudent, student: Student, emergencyContactId?: number) {
    // If emergencyContactId is provided, perform update and return
    if (emergencyContactId) {
        const [existingEmergencyContact] = await db.select().from(emergencyContactModel).where(eq(emergencyContactModel.id, emergencyContactId));
        if (!existingEmergencyContact) {
            return null;
        }

        const [updatedEmergencyContact] = await db.update(emergencyContactModel)
            .set({
...
export async function addPersonalDetails(oldDetails: OldStudent | OldStaff, user: User, personalDetailsId?: number) {
    // If personalDetailsId is provided, perform update and return
    if (personalDetailsId) {
        const [existingPersonalDetails] = await db.select().from(personalDetailsModel).where(eq(personalDetailsModel.id, personalDetailsId));
        ...
        if ('studentCategoryId' in oldDetails) {
            ...
            if (oldDetails.nationalityId) {
                const [nationalityResult] = await mysqlConnection.query(`SELECT * FROM nationality WHERE id = ${oldDetails.nationalityId}`) as [{ id: number, nationalityName: string, code: number }[], any];
            ...
            if (oldDetails.studentCategoryId) {
                const [categoryResult] = await mysqlConnection.query(`SELECT * FROM category WHERE id = ${oldDetails.studentCategoryId}`) as [{ id: number, category: string, code: string, docneeded: boolean | undefined }[], any];
                if (categoryResult.length > 0) {
                    category = await addCategory(categoryResult[0].category, categoryResult[0].code, categoryResult[0].docneeded, db, oldDetails.studentCategoryId);
                }
            }
            let religion: Religion | undefined;
            if (oldDetails.religionId) {
                const [religionResult] = await mysqlConnection.query(`SELECT * FROM religion WHERE id = ${oldDetails.religionId}`) as [{ id: number, religionName: string }[], any];
                if (religionResult.length > 0) {
                    religion = await addReligion(religionResult[0].religionName, db, oldDetails.religionId);
                }
            }
            let motherTongue: LanguageMedium | undefined;
            if (oldDetails.motherTongueId) {
                const [motherTongueResult] = await mysqlConnection.query(`SELECT * FROM mothertongue WHERE id = ${oldDetails.motherTongueId}`) as [{ id: number, mothertongueName: string }[], any];
                if (motherTongueResult.length > 0) {
                    motherTongue = await addLanguageMedium(motherTongueResult[0].mothertongueName, db, oldDetails.motherTongueId);
                }
            }
            ...
            if (existingPersonalDetails.mailingAddressId) {
                await db.update(addressModel)
                    .set({
                        ...
                        localityType: ('localitytyp' in oldDetails && oldDetails.localitytyp?.toUpperCase() === "URBAN") ? "URBAN" : (('localitytyp' in oldDetails && oldDetails.localitytyp?.toUpperCase() === "RURAL") ? "RURAL" : null),
            ...
            if (existingPersonalDetails.residentialAddressId) {
                await db.update(addressModel)
                    ...
                    .where(eq(addressModel.id, existingPersonalDetails.residentialAddressId));
            }

            // Update personal details
            const [updatedPersonalDetails] = await db.update(personalDetailsModel)
                .set({
                    dateOfBirth: oldDetails.dateOfBirth ? (typeof oldDetails.dateOfBirth === 'string' ? oldDetails.dateOfBirth : oldDetails.dateOfBirth.toISOString()) : undefined,
                    gender: oldDetails.sexId === 0 ? undefined : (oldDetails.sexId === 1 ? "MALE" : "FEMALE"),
                    ...
                    aadhaarCardNumber: formatAadhaarCardNumber('aadharcardno' in oldDetails ? (oldDetails.aadharcardno || undefined) : undefined),
    ...
    if ('studentCategoryId' in oldDetails) {
        ...
        const [newPersonalDetails] = await db.insert(personalDetailsModel).values({
            firstName: ('firstName' in oldDetails ? oldDetails.firstName : 'name' in oldDetails ? oldDetails.name : '') || '', // Required field
            mobileNumber: ('mobileNumber' in oldDetails ? oldDetails.mobileNumber : 'phone' in oldDetails ? oldDetails.phone : '') || '', // Required field
            dateOfBirth: oldDetails.dateOfBirth ? (typeof oldDetails.dateOfBirth === 'string' ? oldDetails.dateOfBirth : oldDetails.dateOfBirth.toISOString()) : undefined,
            gender: oldDetails.sexId === 0 ? undefined : (oldDetails.sexId === 1 ? "MALE" : "FEMALE"),
            nationalityId: nationality ? nationality.id : undefined,
...
export async function addBoardUnversity(oldStudent: OldStudent, db: DbType, boardUniversityId?: number): Promise<Board | undefined> {
    ...
    const [existingBoardUniversity] = await db.select().from(boardModel).where(eq(boardModel.name, oldBoardUniversity.boardName.trim().toUpperCase()));
    ...
    const [newBoardUniversity] = await db.insert(boardModel).values({
...
export async function addTransportDetails(oldStudent: OldStudent, student: Student, transportDetailsId?: number) {
    // If transportDetailsId is provided, perform update and return
    if (transportDetailsId) {
        const [existingTransportDetails] = await db.select().from(transportDetailsModel).where(eq(transportDetailsModel.id, transportDetailsId));
        if (!existingTransportDetails) {
            throw new Error(`Transport details with id ${transportDetailsId} not found`);
        }

        // Update transport details
        const [updatedTransportDetails] = await db.update(transportDetailsModel)
            .set({
...
async function processCourseDetails(oldStudent: OldStudent, student: Student, courseDetails: OldCourseDetails) {
    ...
    await getSubjectRelatedFields(courseDetails, programCourse, newAdmissionCourseDetails, student)
    console.log("newAdmissionCourseDetails", newAdmissionCourseDetails);
    return newAdmissionCourseDetails;
}

async function getSubjectRelatedFields(courseDetails: OldCourseDetails, programCourse: ProgramCourse, newAdmissionCourseDetails: AdmissionCourseDetails, student: Student) {
    ...
    for (let i = 0; i < oldCvSubjectSelections.length; i++) {
        ...
        await addAdmSubjectPaperSelection(oldCvSubjectSelection, student.id as number, newAdmissionCourseDetails.id!, foundPaper.id);
...
async function addProgramCourse(stream: Stream | undefined, courseDetails: OldCourseDetails, course: Course, courseType: CourseType) {
...
async function addMeritList(oldMeritListId: number | null) {
    ...
    const [foundMeritList] = await db
        ...
        .from(meritListModel)
        .where(
            ilike(meritListModel.name, meritListRow.name.trim().toUpperCase()),
        )
    ...
    const [foundAdmSbjPprSelection] = await db
        ...
        .where(
            and(
                ...
                eq(admSubjectPaperSelectionModel.admissionCourseDetailsId, admissionCourseDetailsId),
