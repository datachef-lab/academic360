  ...
  try {
    ...
    if (!studentId) {
      res
        .status(400)
        .json(
          new ApiResponse(400, "BAD_REQUEST", null, "Student ID is required."),
        );
    }
    ...
    res
      .status(200)
      .json(
        new ApiResponse(200, "SUCCESS", [], "Report fetched successfully!"),
      );
  } catch (error) {
...
export const getAllReports = async (
  ...
  try {
    ...
    if (!reportsData) {
      res
        .status(404)
        .json(new ApiResponse(404, "NOT_FOUND", null, "No reports found."));
    }
