...
import { createFamilySchema } from "../models/family.model.js";
...
export const getFamilyByStudentId = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
export const deleteFamilyById = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { id } = req.params;
...
export const deleteFamilyByStudentId = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { studentId } = req.params;
...
export const getAllFamiliesController = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const families = await getAllFamilies();
    res.status(200).json(new ApiResponse(200, "SUCCESS", families, "Fetched all families successfully!"));
  } catch (error) {
    handleError(error, res, next);
  }
};
