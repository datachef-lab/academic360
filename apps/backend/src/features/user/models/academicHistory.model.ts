...
export const academicHistoryModel = pgTable("academic_history", {
    ...
    studentId: integer().notNull().references(() => studentModel.id,{ onDelete: "cascade" }),
    lastInstitutionId: integer().references(() => institutionModel.id,{ onDelete: "set null" }),
    lastBoardUniversityId: integer().references(() => boardUniversityModel.id,{ onDelete: "set null" }),
    studiedUpToClass: integer(),
    passedYear: integer(),
    specialization: varchar({ length: 255 }),
    lastResultId: integer().references(() => boardResultStatusModel.id,{ onDelete: "set null" }),
