...
import { ProfileInfo, StaffDto, StudentDto, UserDto, PersonalDetailsDto, HealthDto, AccommodationDto, AddressDto } from "@repo/db/index.js";
import { AdmissionGeneralInfoDto, ApplicationFormDto } from "@repo/db/dtos/admissions";
import { admissionGeneralInfoModel, admissionAcademicInfoModel, admissionAdditionalInfoModel, admissionCourseDetailsModel, applicationFormModel, accommodationModel, emergencyContactModel, healthModel, personalDetailsModel, nationalityModel, religionModel, categoryModel, languageMediumModel, addressModel, countryModel, stateModel, cityModel, bloodGroupModel, boardModel, ApplicationForm, PersonalDetails, Health, Accommodation, Address, AdmissionGeneralInfo, AdmissionAcademicInfo, AdmissionAdditionalInfo, AdmissionCourseDetails } from "@repo/db/schemas";
...
export async function findProfileInfo(userId: number): Promise<ProfileInfo | null> {
    // Determine user type first
    const [coreUser] = await db.select().from(coreUserModel).where(eq(coreUserModel.id, userId));
    ...
    const student = isStudent ? await studentService.findStudentByUserId(userId) : null;

    // Application form
    const applicationFormId = student?.applicationFormAbstract?.id ?? null;
    const applicationForm = applicationFormId
        ? (await db.select().from(applicationFormModel).where(eq(applicationFormModel.id, applicationFormId)))[0]
        : null;
    ...
    const generalInfo = applicationForm?.id
        ? (await db
            ...
            .where(eq(admissionGeneralInfoModel.applicationFormId, applicationForm.id)))[0]
        : null;

    const [personalDetails, healthDetails, emergencyContactDetails, accommodationDetails] = await Promise.all([
        generalInfo?.personalDetailsId
            ? (await db.select().from(personalDetailsModel).where(eq(personalDetailsModel.id, generalInfo.personalDetailsId)))[0]
            : null,
        generalInfo?.healthId
            ? (await db.select().from(healthModel).where(eq(healthModel.id, generalInfo.healthId)))[0]
            : null,
        generalInfo?.emergencyContactId
            ? (await db.select().from(emergencyContactModel).where(eq(emergencyContactModel.id, generalInfo.emergencyContactId)))[0]
            : null,
        generalInfo?.accommodationId
            ? (await db.select().from(accommodationModel).where(eq(accommodationModel.id, generalInfo.accommodationId)))[0]
            : null,
    ]);
    ...
    const academicInfo = applicationForm?.id
        ? (await db
            ...
            .where(eq(admissionAcademicInfoModel.applicationFormId, applicationForm.id)))[0] ?? null
        : null;
    ...
    const additionalInfo = applicationForm?.id
        ? (await db
            ...
            .where(eq(admissionAdditionalInfoModel.applicationFormId, applicationForm.id)))[0] ?? null
        : null;
    ...
    const courseApplications = applicationForm?.id
        ? await db
            ...
            .where(eq(admissionCourseDetailsModel.applicationFormId, applicationForm.id))
        : [];

    const applicationFormDto: ApplicationFormDto | null | undefined = isStudent && applicationForm
        ? await mapApplicationFormToDto(applicationForm, {
            generalInfo,
            personalDetails: personalDetails ? await mapPersonalDetailsToDto(personalDetails) : null,
            health: healthDetails ? await mapHealthToDto(healthDetails) : null,
            emergencyContact: emergencyContactDetails ?? null,
            accommodation: accommodationDetails ? await mapAccommodationToDto(accommodationDetails) : null,
            academicInfo,
            additionalInfo,
            courseApplications,
        })
        : undefined;
    const personalDetailsDto: PersonalDetailsDto | null = personalDetails ? await mapPersonalDetailsToDto(personalDetails) : null;
    const healthDto: HealthDto | null = healthDetails ? await mapHealthToDto(healthDetails) : null;
    const accommodationDto: AccommodationDto | null = accommodationDetails ? await mapAccommodationToDto(accommodationDetails) : null;
...
async function mapApplicationFormToDto(
    ...
    const [blockedBy, admApprovedBy] = await Promise.all([
        app.blockedBy ? db.select().from(coreUserModel).where(eq(coreUserModel.id, app.blockedBy)).then(r => r[0] ?? null) : null,
        app.admApprovedBy ? db.select().from(coreUserModel).where(eq(coreUserModel.id, app.admApprovedBy)).then(r => r[0] ?? null) : null,
    ]);
    ...
    return {
        ...
        blockedBy,
        admApprovedBy,
        generalInfo: rel.generalInfo ? await mapGeneralInfoToDto(rel.generalInfo, rel) : null,
        courseApplication: rel.courseApplications && rel.courseApplications.length
            ? await Promise.all(rel.courseApplications.map(mapCourseDetailsToDto))
            : [],
        academicInfo: rel.academicInfo ? await mapAcademicInfoToDto(rel.academicInfo) : null,
        additionalInfo: rel.additionalInfo ? await mapAdditionalInfoToDto(rel.additionalInfo) : null,
        paymentInfo: null,
    };
}

async function mapGeneralInfoToDto(g: AdmissionGeneralInfo, rel: {
    personalDetails: PersonalDetailsDto | null;
    health: HealthDto | null;
...
async function mapAcademicInfoToDto(a: AdmissionAcademicInfo): Promise<import("@repo/db/dtos/admissions").AdmissionAcademicInfoDto> {
    const [board, lastSchoolAddress] = await Promise.all([
        a.boardId ? db.select().from(boardModel).where(eq(boardModel.id, a.boardId)).then(r => r[0] ?? null) : null,
...
async function mapAdditionalInfoToDto(ad: AdmissionAdditionalInfo): Promise<import("@repo/db/dtos/admissions").AdmissionAdditionalInfoDto> {
    return {
        ...ad,
...
async function mapCourseDetailsToDto(cd: AdmissionCourseDetails): Promise<import("@repo/db/dtos/admissions").AdmissionCourseDetailsDto> {
    return {
        ...cd,
        stream: undefined,
...
async function mapPersonalDetailsToDto(p: PersonalDetails): Promise<PersonalDetailsDto> {
    const [nationality, religion, category, motherTongue, mailingAddress, residentialAddress] = await Promise.all([
        p.nationalityId ? db.select().from(nationalityModel).where(eq(nationalityModel.id, p.nationalityId)).then(r => r[0] ?? null) : null,
        p.religionId ? db.select().from(religionModel).where(eq(religionModel.id, p.religionId)).then(r => r[0] ?? null) : null,
        p.categoryId ? db.select().from(categoryModel).where(eq(categoryModel.id, p.categoryId)).then(r => r[0] ?? null) : null,
        p.motherTongueId ? db.select().from(languageMediumModel).where(eq(languageMediumModel.id, p.motherTongueId)).then(r => r[0] ?? null) : null,
...
async function mapHealthToDto(h: Health): Promise<HealthDto> {
    const blood = h.bloodGroupId ? await db.select().from(bloodGroupModel).where(eq(bloodGroupModel.id, h.bloodGroupId)).then(r => r[0] ?? null) : null;
    return {
        ...h,
...
async function mapAccommodationToDto(a: Accommodation): Promise<AccommodationDto> {
...
async function fetchAddressDto(addressId: number): Promise<AddressDto | undefined> {
    const [addr] = await db.select().from(addressModel).where(eq(addressModel.id, addressId));
    if (!addr) return undefined;
    const [country, state, city] = await Promise.all([
        addr.countryId ? db.select().from(countryModel).where(eq(countryModel.id, addr.countryId)).then(r => r[0] ?? null) : null,
        addr.stateId ? db.select().from(stateModel).where(eq(stateModel.id, addr.stateId)).then(r => r[0] ?? null) : null,
        addr.cityId ? db.select().from(cityModel).where(eq(cityModel.id, addr.cityId)).then(r => r[0] ?? null) : null,
    ]);
    return {
        ...addr,
