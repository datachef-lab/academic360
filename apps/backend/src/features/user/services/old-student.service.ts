...
export async function processOldStudent(
    oldAdmStudentPersonalDetails: OldAdmStudentPersonalDetail,
) {
...
export async function addSection(oldSectionId: number): Promise<Section | undefined> {
    const [rows] = (await mysqlConnection.query(
...
export async function upsertHealth(
    ...
    if (admissionGeneralInfoId && userId) {
        throw new Error("Either of admissionGeneralInfoId or userId is required, not both!");
    }
    ...
    if (userId) {
        existingHealth = (await db
            .select()
            .from(healthModel)
            .where(
                eq(healthModel.userId, userId)
            ))[0];
    }
    else {
        existingHealth = (await db
            ...
            .from(healthModel)
            .where(
                eq(healthModel.admissionGeneralInfoId, admissionGeneralInfoId!)
            ))[0];
    }
...
export async function upsertEmergencyContact(emergencyContact: EmergencyContact, userId?: number, admissionGeneralInfoId?: number) {
    if (userId && admissionGeneralInfoId) {
        throw new Error("Either of userId or admissionGeneralInfoId is required, not both!");
    }

    let existingEmergencyContact: EmergencyContact | undefined = undefined;
    if (userId) {
        existingEmergencyContact = (await db
            .select()
            .from(emergencyContactModel)
            .where(eq(emergencyContactModel.userId, userId)))[0];
    }
    else {
        existingEmergencyContact = (await db
            .select()
            .from(emergencyContactModel)
            .where(eq(emergencyContactModel.admissionGeneralInfoId, admissionGeneralInfoId!)))[0];
    }
...
export async function upsertPersonalDetails(
    ...
    const emergencyResidentialNumber = isStaff(oldDetails)
        ? oldDetails.emergencytelmobile || oldDetails.emergencytellandno || undefined
    ...
    if (isAdmStudent(oldDetails)) {
        existingPersonalDetails = (await db
            .select()
            .from(personalDetailsModel)
            .where(
                eq(personalDetailsModel.admissionGeneralInfoId, admissionGeneralInfoId!))
        )[0];
    }
    else {
        existingPersonalDetails = (await db
            ...
            .from(personalDetailsModel)
            .where(
                eq(personalDetailsModel.userId, userId!)
            ))[0];
    }
    ...
    if (existingPersonalDetails) {
        ...
        const [newPersonalDetails] = await db
            ...
            .values({
                admissionGeneralInfoId: isAdmStudent(oldDetails) ? admissionGeneralInfoId : undefined,
...
async function upsertPersonalDetailsAddress(oldDetails: OldAdmStudentPersonalDetail | OldStaff, personalDetailsId: number) {
