...
import { z } from "zod";
import { PersonDto } from "@academic/db/index.js";
...
export async function findPersonById(id: number): Promise<PersonDto | null> {
  const [foundPerson] = await db
    .select()
    .from(personModel)
    .where(eq(personModel.id, id));
...
export async function getAllPersons(): Promise<PersonDto[]> {
  ...
  return formatted.filter((p): p is PersonDto => !!p);
}
...
export async function removePerson(id: number): Promise<boolean | null> {
  ...
  //   await db
  //     .delete(familyModel)
  //     .where(eq(familyModel.fatherDetailsId, id))
  //     .returning();
  //   await db
  //     .delete(familyModel)
  //     .where(eq(familyModel.motherDetailsId, id))
  //     .returning();
  ...
  //       .delete(addressModel)
  //       .where(eq(addressModel.id, foundPerson.officeAddressId))
  //       .returning();
...
export async function removePersonByAddressId(
  ...
  //     .select()
  //     .from(personModel)
  //     .where(eq(personModel.officeAddressId, officeAddressId));
  //   if (!foundPerson) {
  //     return null;
  //   }
  //   // Delete the person: -
  ...
  //   await db
  //     .delete(familyModel)
  //     .where(eq(familyModel.fatherDetailsId, foundPerson.id))
  //     .returning();
  //   await db
  //     .delete(familyModel)
  //     .where(eq(familyModel.motherDetailsId, foundPerson.id))
  //     .returning();
  //   // Step 2: Delete the person
  //   const [deletedPerson] = await db
  //     .delete(personModel)
  //     .where(eq(personModel.id, foundPerson.id))
  //     .returning();
  //   if (!deletedPerson) {
  //     return false;
  //   }
...
export async function findPersonByFamilyId(
  familyId: number,
): Promise<PersonDto[]> {
  const persons = await db
    .select()
    .from(personModel)
    .where(eq(personModel.familyId, familyId));
  const formatted = await Promise.all(persons.map(personResponseFormatted));
  return formatted.filter((p): p is PersonDto => !!p);
}

export async function personResponseFormatted(
  person: Person,
): Promise<PersonDto | null> {
  if (!person) return null;
  const { occupationId, qualificationId, ...props } = person;
  const formattedPerson: PersonDto = { ...props } as PersonDto;
  if (occupationId) {
    (formattedPerson as any).occupation =
      await findOccupationById(occupationId);
  }
  if (qualificationId) {
    (formattedPerson as any).qualification =
      await findQualificationById(qualificationId);
  }
  //   if (officeAddressId) {
  //     (formattedPerson as any).officeAddress =
  //       await findAddressById(officeAddressId);
  //   }
