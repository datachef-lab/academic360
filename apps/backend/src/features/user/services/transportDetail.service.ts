...
export async function findTransportDetailsById(
  id: number,
): Promise<TransportDetails | null> {
  const [foundTransportDetail] = await db
    .select()
    .from(transportDetailsModel)
    .where(eq(transportDetailsModel.id, id));
  return foundTransportDetail || null;
}

export async function findTransportDetailsByStudentId(
  studentId: number,
): Promise<TransportDetails | null> {
...
export async function updateTransportDetails(
  ...
  const [updatedTransportDetail] = await db
...
export async function removeTransportDetails(
  id: number,
  ...
  const [deletedTransportDetail] = await db
    .delete(transportDetailsModel)
    .where(eq(transportDetailsModel.id, id))
    .returning();
...
export async function removeTransportDetailsByStudentId(
  studentId: number,
): Promise<boolean | null> {
  // const [foundTransportDetail] = await db.select().from(transportDetailsModel).where(eq(transportDetailsModel.studentId, studentId));
  // if (!foundTransportDetail) {
