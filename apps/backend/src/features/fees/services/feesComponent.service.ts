...
export const getFeesComponentById = async (id: number) => {
  try {
    const feesComponent = await db
      .select()
      .from(feesComponentModel)
      .where(eq(feesComponentModel.id, id));
...
export const createFeesComponent = async (feesComponent: FeesComponent) => {
  try {
    const newFeesComponent = await db
      .insert(feesComponentModel)
      .values(feesComponent)
      .returning();
...
export const updateFeesComponent = async (
  id: number,
  feesComponent: FeesComponent,
) => {
  try {
    const updatedFeesComponent = await db
...
export const deleteFeesComponent = async (id: number) => {
  try {
    const deletedFeesComponent = await db
